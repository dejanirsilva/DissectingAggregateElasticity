...
\alpha_{0,D,t} = \overline{\alpha}_{p,t}\,\alpha_{m,D,t},
\; \alpha_{0,B,t} = \overline{\alpha}_{p,t}\,\alpha_{m,B,t},
\; \alpha_{0,F,t} = 0\},
\end{equation}
where $\alpha_{m,k,t} \equiv \frac{P_{k,t}}{P_{Y,t}}$ is the market weight of asset $k \in \{D,B\}$.
...
The key step is to derive the optimality conditions that govern portfolio choice and consumption decisions and their implications for asset pricing.
We then establish a version of the \textit{mutual fund theorem} in our economy with frictions.
This result shows that, despite the presence of multiple traded assets, equilibrium allocations can be summarized in terms of aggregate risk exposures.
As a result, the equilibrium reduces to a tractable representation that will be central for our analysis of aggregate market elasticity.
...
Importantly, it also depends on the investor's portfolio choice $\alpha_{j,t}$ through risk exposure.
...
The second condition states that the wealth-weighted average risk exposure equals the aggregate risk exposure of the endowment claim.
...
\paragraph{Interest rate.} Combining the market clearing condition for goods and the expression for the consumption-wealth ratio, we can solve for the interest rate:   
\begin{align}\label{eq:interest_rate}
    r_t &= \rho + \psi^{-1} (\mu_{P_Y,t} + \xi_t )
...
Let $p_{D,t} \equiv P_{D,t}/D_t$ and $p_{B,t} \equiv P_{B,t}/B_t$ denote the price-dividend ratios of the dividend and bond claims, respectively. 
The price-dividend ratio of the aggregate claim, $p_{Y,t} \equiv P_{Y,t}/Y_t$, is given by $p_{Y,t} = s_t p_{D,t} + (1-s_t) p_{B,t}$.

The pricing condition for each claim is given by:
\begin{equation}\label{eq:pricing_cond}
    \frac{1}{p_{k,t}} = r_t + \pi_{k,t} - (\mu_{k,t} + \mu_{p_k,t} + \sigma \sigma_{p_k,t}^\top),
\end{equation}
where $\pi_{k,t}= \sigma_{R_k,t}\eta_t^\top$ is the risk premium on claim $k$, $\mu_{p_k,t}$ is the drift of the price-dividend ratio, and $\sigma_{p_k,t}$ is the diffusion of the price-dividend ratio.


\paragraph{State variables.} 
We have two endogenous state variables, $x_{1,t}$ and $x_{2,t}$, which summarize the wealth distribution in this economy, and two exogenous state variables, $\overline{\alpha}_{p,t}$ and $s_t$, which capture the portfolio choice of passive investors and the dividend share, respectively. 
Hence, the aggregate state vector is $X_t = (x_t, \overline{\alpha}_{p,t}, s_t)$, where $x_t = (x_{1,t}, x_{2,t})$. 
The law of motion of $\overline{\alpha}_{p,t}$ is given by \eqref{eq:alpha_p_process} and the law of motion of $s_t$ is given by \eqref{eq:s_process}. 

By It\^{o}'s lemma, the law of motion for $x_{j,t}$ is
\begin{align*}
    \frac{dx_{j,t}}{x_{j,t}} 
    &= \Big[ r_t + \pi_t^\top \alpha_{j,t} - c_{j,t} - \mu_{P_Y,t} 
        - (\sigma_{j,t} - \sigma_{R_Y,t}) \sigma_{R_Y,t}^\top 
        + \kappa \frac{\omega_j - x_{j,t}}{x_{j,t}} \Big] dt  + (\sigma_{j,t} - \sigma_{R_Y,t})\, dZ_t.
\end{align*}
...
Return volatility has both an exogenous and an endogenous component, $\sigma_{R_k,t} = \sigma_{k,t} + \sigma_{p_k,t}$, where $\sigma_{k,t}$ reflects cash-flow volatility and $\sigma_{p_k,t}$ arises from movements in the price-dividend ratio $p_{k,t}$ for asset $k \in \{D,B\}$. 
By It\^{o}'s lemma,  
\begin{equation}\label{eq:_endogenous_vol}
    \sigma_{p_k,t} = \frac{p_{k,x}(X_t)}{p_{k,t}(X_t)} \sigma_{x,t} 
    + \frac{p_{k,\overline{\alpha}_p}(X_t)}{p_{k,t}(X_t)} \sigma_{\overline{\alpha}_p,t} + \frac{p_{k,s}(X_t)}{p_{k,t}(X_t)} \sigma_{s,t}.
\end{equation}

Equation~\eqref{eq:_endogenous_vol} shows that endogenous volatility depends on the response of the price-dividend ratio to changes in i) the wealth distribution, ii) the passive portfolio, and iii) the dividend share.
The first term is standard in heterogeneous-agent models \citep[see][]{panageas2020implications} and the third term is typical of models with multiple assets, while the second arises only with exogenous portfolio flow shocks and is quantitatively relevant when markets are sufficiently inelastic (i.e., when $\frac{p_{\overline{\alpha}_p}(X_t)}{p(X_t)}$ is large). Thus, \eqref{eq:_endogenous_vol} links market elasticity to return volatility $\|\sigma_{R_k,t}\|$.

Equation~\eqref{eq:_endogenous_vol} plays a key role connecting portfolio flows, market elasticity, and excess volatility.

Given $\sigma_{R_{k},t}$, we can recover $\sigma_{R_Y,t}$ using the condition $\sigma_{R_Y,t} = \sum_{k=D,B} \frac{P_{k,t}}{P_{Y,t}} \sigma_{R_{k},t}$.
It remains to determine $\sigma_{R_F,t}$.  
We assume that $\sigma_{R_F,t}$ does not load on the dividend-share shock, so $\sigma_{R_F,t,3} = 0$.
We also assume that $\sigma_{R_F,t}$ is orthogonal to the aggregate claim, so $\sigma_{R_F,t}\,\sigma_{R_Y,t}^\top = 0$, and 
we adopt the normalization $\|\sigma_{R_F,t}\| = \|\sigma_{R_Y,t}\|$. 
These three conditions uniquely pin down $\sigma_{R_F,t}$ given $\sigma_{R_Y,t}$.

\paragraph{Markov equilibrium.} 
Equations \eqref{eq:market_price_of_risk} and \eqref{eq:interest_rate} allow us to express $\lambda_t$ and $r_t$ in terms of $c_{j,t}$, $p_{k,t}$, and their derivatives, once $(\mu_{c_j,t}, \mu_{p_k,t})$ and $(\sigma_{c_j,t}, \sigma_{p_k,t})$ are written as functions of $(\mu_{X,t}, \sigma_{X,t})$ via It\^{o}'s lemma. 
Substituting these into \eqref{eq:HJB_complete} and \eqref{eq:pricing_cond}, together with the laws of motion for the state variables, yields a system of five PDEs in $\{c_j(X_t)\}_{j=0}^2$ and $p(X_t)$. These functions depend on four state variables: the two active wealth shares $x_{1,t}$ and $x_{2,t}$, the passive portfolio share $\overline{\alpha}_{p,t}$, and the dividend share $s_t$.  


\subsection{Mutual fund separation equilibrium}\label{sec:pricing_claims}
