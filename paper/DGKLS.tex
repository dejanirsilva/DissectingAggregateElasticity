...
We focous on the case where $\sigma=(\sigma^1,0,0)$ for aggregate endowment risk, $\sigma_{\alpha_p}=(\sigma_{\alpha_p}^1,\sigma_{\alpha_p}^2,0)$ for the passive-share process, and $\sigma_s=(\sigma_{s}^1,0,\sigma_{s}^3)$ for the dividend share process.
...
\begin{prop}[Block recursivity and the mutual fund theorem]\label{prop:mft}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \begin{align}
            ...
            \frac{1}{p_{Y}} &= r + \eta\,(\sigma + \sigma_{p_Y})^\top - \big(\mu + \mu_{p_Y} + \sigma \, \sigma_{p_Y}^\top\big),        
    \end{align}
...
This assumption follows \citet{MenzlySantosVeronesi2004}, but differs from, e.g., the two-trees model of \citet{cochraneLongstaffSantaClara2008}, where the dividend share directly affects the aggregate endowment volatility.
...
Without the derivative, investors would need to deviate from market-capitalization weights to adjust the relative exposure to these risks, at the cost of bearing dividend-share risk.
...
The elasticity is determined by the coupled PDE system discussed in Proposition~\ref{prop:mft}, which generally has no closed-form solution. To study its determinants, we extend the perturbation method of \citet{kogan2001risk} and derive asymptotic closed-form approximations.
...
\[
    ...
    \end{bmatrix},
\]
where $f_t \equiv \sum_{j=0}^J x_{j,t} f_{j,t}$ and $\chi_{0,t} \equiv \sum_{j=0}^J x_{j,t} \chi_{j,0}$.
