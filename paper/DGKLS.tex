...
We first derive the optimality conditions that govern portfolio choice and consumption decisions, and then outline their implications for asset pricing.
...
The second term captures intertemporal substitution through expected capital gains and changes in investment opportunities.
The third term captures precautionary savings and depends on the distribution of risk exposures across investors.
The final term reflects the effect of risk premia on the interest rate.

This expression highlights that the interest rate is determined jointly by risk premia and the allocation of risk across investors, providing the key general equilibrium link between portfolio flows and saving behavior.
...
Equation~\eqref{eq:_endogenous_vol} shows that endogenous volatility depends on the sensitivity of the price-dividend ratio to three variables: (i) the wealth distribution, (ii) the passive portfolio share, and (iii) the dividend share.
...
We show next that the system of PDEs satisfies an important \textit{block-recursive} structure.
...
This provides a substantial simplification of the equilibrium characterization, as the economy behaves as if it were a single-claim economy with a single claim on the aggregate endowment and a derivative.
At the same time, by exploring the block-recursive structure of the system of PDEs, we can still obtain the implications of portfolio flows on the aggregate market elasticity for the dividend claim. 

Another important implication of the block-recursive structure is that a version of the \textit{mutual fund theorem} holds in our economy.
Despite the presence of financial frictions in our dynamic model, investors optimally hold risky claims at market-capitalization weights.
