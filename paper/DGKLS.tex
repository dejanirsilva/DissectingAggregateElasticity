...
\end{equation}
where $\mu \in \mathbb{R}$ and $\sigma \in \mathbb{R}^{1\times d}$ are constants.
...
This decomposition allows us to distinguish between claims with different risk exposures, which will be central when considering the implications for price impact.
...
\paragraph{Stochastic discount factor.} Absence of arbitrage implies the existence of a stochastic discount factor (SDF), $\eta_t$, which prices all assets in the economy:
\begin{equation}
...
For example, consider $\sigma=(\sigma_1,0,0)$ for aggregate endowment risk, $\sigma_{\alpha_p}=(\sigma_{\alpha_p,1},\sigma_{\alpha_p,2},0)$ for the passive-share process, and $\sigma_s=(\sigma_{s,1},0,\sigma_{s,3})$ for the dividend share process.
...
Active investors optimally rebalance their portfolios, choosing allocations across the dividend claim, the bond claim, and the derivative, $(\alpha_{j,D,t}, \alpha_{j,B,t}, \alpha_{j,F,t})$.
...
\end{equation}
where $\Sigma_{j,t} = \alpha_{j,D,t}\,\sigma_{R_D,t} + \alpha_{j,B,t}\,\sigma_{R_B,t} + \alpha_{j,F,t}\,\sigma_{R_F,t}$ is the investor's total risk exposure and $\|\cdot\|$ denotes the Euclidean norm.
...
An investor of type $j = 0,1,2$ chooses consumption $C_{j,t}$ and portfolio allocations $\alpha_{j,t}$, where $\alpha_{j,t} \equiv [\alpha_{j,D,t}, \alpha_{j,B,t}, \alpha_{j,F,t}]^\top$, to solve the following problem:
...
\begin{equation}\label{eq:budget_constraint}
    dW_{j,t} = \left[r_t + \pi_{t}^\top \,\alpha_{j,t} - \frac{C_{j,t}}{W_{j,t}}\right] W_{j,t}\,dt 
    + \Sigma_{j,t}\, W_{j,t}\, dZ_t,
\end{equation}
with $W_{j,t} \geq 0$ and $\alpha_{j,t} \in \Omega_{j,t}$,
where $W_{j,t}$ denotes the investor's wealth and $\Omega_{j,t}$ is the investment set for type-$j$ investor.

Portfolio allocations are restricted to lie in an investment set $\Omega_{j,t}$, which depends on investor type and captures differences in financial constraints across investors.\footnote{
...
\alpha_{0,D,t} = \overline{\alpha}_{p,t}\,\omega_{D,t},
\; \alpha_{0,B,t} = \overline{\alpha}_{p,t}\,\omega_{B,t},
\; \alpha_{0,F,t} = 0\}.
\end{equation}
...
\begin{equation}
\Omega_{2,t} = \{\alpha_{2,t}: \|\Sigma_{2,t}\| \leq \overline{\sigma}\}.
\end{equation}
...
These include the aggregate endowment $Y$, the dividend share $s$, the prices $P_D$ and $P_B$, the risk-free rate $r$, the weight $\overline{\alpha}_p$, and, for each investor $j \in \{0,1,2\}$, wealth $W_j$, consumption $C_j$, and portfolio allocations $\alpha_j$.
...
\item Given prices $\{P_{D,t}, P_{B,t}, r_t\}$ and the passive share $\overline{\alpha}_{p,t}$, the choices $(C_{j,t},\alpha_{j,t})$ solve investor $j$'s problem in \eqref{eq:inv-problem}.
...
The notional position in the derivative is $F_{j,t} = \alpha_{j,F,t} W_{j,t}$.\footnote{
    As in a futures contract, there is no initial cash exchange, so wealth invested in the risk-free asset equals total wealth minus the amount invested in the dividend and bond claims.}
...
We first present the investors' optimality conditions, then establish a version of the \textit{mutual fund theorem} in our economy with frictions, which simplifies the equilibrium characterization.
