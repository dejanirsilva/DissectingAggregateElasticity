...
\begin{table}[!t]
    ...
    Preference parameters ($\psi$, $\gamma_j$, $\rho$) and sectoral masses ($\omega_j$) are chosen to match average sectoral wealth shares and sectoral ``betas'' from time-series regressions of risky-asset holdings on aggregate risky supply.
...
We discipline the calibration with two groups of moments.
The first group targets the cross-sectional distribution of risky holdings.
We choose the sector masses $\omega_j$ and the risk-aversion coefficients $\gamma_j$ jointly so that the model matches the average sectoral wealth shares in the Flow of Funds and the sectoral ``betas,'' defined as the slope coefficient from a time-series regression of sector~$j$'s dollar risky-asset holdings on aggregate risky supply.
These sector betas are the empirical counterpart of the comovement between sector~$j$'s dollar position and the aggregate; we compute them for the two active sectors, $j\in\{c,u\}$.
The average risk aversion $\overline{\gamma}$ is fixed at $6.15$, a standard value in the macro-finance literature.
...
The second group targets the passive-demand process.
Its long-run mean $\overline{\alpha}$ is set to the household-weighted aggregate implied by our FoF aggregation, which yields $\overline{\alpha}=0.52$.
...
Following Appendix~D3 of \citet{gabaix2020search}, we scale each sector's dollar equity flow by lagged aggregate market capitalization, purge mechanical revaluation, and aggregate gross flows across sectors, dividing by two to avoid double counting.
...
Following \citet{LettauWachter2011}, we set the correlation of dividend-share shocks with the fundamental at $0.3$ and the dividend-share volatility at $11\%$.

\begin{table}[!t]
    \centering
    \input{./tables/simulation.tex}
    \vspace{2mm}
    \captionsetup{font=footnotesize}
    ...
    The price impact is estimated as the slope coefficient $\hat{b}_1$ of the regression $d\log(P/D)_t = a_0 + b_1\,\overline{\alpha}_{p,t}^{s} + e_t$, where $\overline{\alpha}_{p,t}^{s}$ is the innovation to passive demand.
    ...
    Rates, volatilities, and asset shares are reported in percent; Asset/Equity ratios, price impacts, and their standard errors are unit-free.
    Standard errors for price-impact estimates are reported in separate rows.
...
In our setting, the passive-demand innovation $\overline{\alpha}_{p,t}^{s}$ is observed and exogenous by construction (the passive share follows the exogenous CIR process in equation~\eqref{eq:alpha_p_process}), so in simulated data the OLS slope of equation~\eqref{eq:price_impact_regression} directly identifies the model-implied counterpart of the GIV estimand without requiring an instrument.
...
\begin{table}[!t]
    \centering
    \resizebox{0.92\linewidth}{!}{\input{./tables/counterfactuals.tex}}
    \vspace{2mm}
    \captionsetup{font=footnotesize}
