...
To provide an analytical characterization of aggregate market elasticity, we extend the perturbation method of \citet{kogan2001risk} and derive asymptotic closed-form approximations.
...
Given the results in Proposition~\ref{prop:mft}, we can express the holdings of asset $k\in\{D,B\}$ for investor $j$ as follows:
\begin{equation}
    Q_{j,k,t} = \alpha_{j,k,t} \frac{W_{j,t}}{P_{k,t}} = \alpha_{j,Y,t} \frac{x_{j,t}}{\omega_j},
\end{equation}
where $x_{j,t}$ is the wealth share of investor $j$ and $\alpha_{j,Y,t}$ is the effective exposure to the aggregate claim.

The expression above implies that quantities are equalized across assets, an implication of the mutual fund theorem, so we can define the demand for risky assets as $Q_{j,t} \equiv Q_{j,k,t}$.

In recursive form, investor $j$’s demand for the risky asset is then given by
\[
    Q_j(x, \overline{\alpha}_p;\epsilon)=\alpha_{j,Y}(x, \overline{\alpha}_p;\epsilon)\,\frac{x_j}{\omega_j},
\]

Define the proportional deviation from the benchmark as
\begin{equation}
    q_{j,t}\;\equiv\;\frac{Q_j(x_t, \overline{\alpha}_{p,t};\epsilon)- Q_j(x_t, \overline{\alpha}_{p,t};0)}{Q_j(x_t, \overline{\alpha}_{p,t};0)}
    \;=\;\alpha_{j,Y,1}(x_t, \hat{\alpha}_{p,t})\,\epsilon+\mathcal{O}(\epsilon^2),
\end{equation}
where $\alpha_{j,Y,1}(x_t, \hat{\alpha}_{p,t})$ is the first-order correction to the portfolio weight.  

From Equation~\eqref{eq:alpha_Y}, we have that portfolio weight depends on the return volatility, $\sigma_{R_Y}$, and the risk exposure, $\sigma_{j}$.
The return volatility is given by $\sigma_{R_Y} = \sigma + \sigma_{p_Y}$, where the volatility of the valuation ratio is second order in $\epsilon$:
\[
    \sigma_{p_Y}=\frac{p_{Y,\hat{X}}(\hat{X})}{p(\hat{X})}\,\sigma_{\hat{X}}(\hat{X})=\mathcal{O}(\epsilon^2), 
\]
since $p_{Y,\hat{X}}(\hat{X})$, $\mu_{\hat{X}}$ and $\sigma_{\hat{X}}$ are first order in $\epsilon$, as shown in Appendix~\ref{app:derivations-perturbation-solution}.

Expanding the risk exposure for active investors $j=1,2$ in Equation~\eqref{eq:alpha_Y} gives
\begin{equation}
    \sigma_{j,1} = \frac{\eta_1}{\gamma} - \frac{\eta_0}{\gamma} (\hat{\gamma}_j+ \hat{\nu}_{j}),
\end{equation}
where $\eta_1$ is the first-order correction to the market price of risk and $\hat{\nu}_j$ is the first-order correction to the Lagrange multiplier on the leverage constraint.

Combining the results above with Equation~\eqref{eq:alpha_Y}, we obtain the portfolio share of active investors:
\begin{equation}
    \alpha_{j,Y,1} = \frac{\pi_{Y,1}}{\gamma \|\sigma\|^2} - (\hat{\gamma}_j+ \hat{\nu}_{j}),
\end{equation}
using the fact that $\pi_{Y,1} = \eta_1 \sigma^\top$.
For passive investors, we have $\alpha_{0,Y,1} = \hat{\alpha}_{p}$.

From the pricing condition Equation~\eqref{eq:pde_p_Y}, we can express the risk premium in terms of the interest rate and the price of the aggregate claim:
\begin{equation}
    \hat{\pi}_{Y,t}=-\,\hat{r}_t-\frac{1}{p^{*}}\hat{p}_{Y,t}+\mathcal{O}(\epsilon^2),
\end{equation}
where $\hat{\pi}_{Y,t}\equiv \pi_{Y,t}-\pi_{Y}^{*}$, $\hat{r}_t\equiv r_t-r^{*}$, and $\hat{p}_{Y,t}\equiv \frac{p_{Y,t}-p_{Y}^{*}}{p_{Y}^{*}}$, using $\mu_{p_{Y},t}=\mathcal{O}(\epsilon^2)$.

Substituting into the expression for $q_{j,t}$, we obtain the first-order demand system is
...
Equation~\eqref{eq:demand_risky} shows that the first-order approximation to the dynamic heterogeneous agent model in Section~\ref{sec:model} provides microfoundations for the demand system in Section~\ref{sec:simple}.
...
In addition, the model generates further demand shifts from occasionally binding leverage constraints faced by active investors.
...
\begin{equation}
    ...
    \;+\; \mathcal{O}(\epsilon^2).
\end{equation}
Consistent with Assumption~\ref{assump:c_w_property}, the risk-free rate and the risk premium enter symmetrically at first order.
...
Hence, conditional on $p_t$, the consumption-wealth ratio is independent of $r_t$, as in Section~\ref{sec:simple}.
Its slope with respect to $p_t$ depends on the EIS: it rises with $p_t$ when $\psi>1$, the standard assumption in macro-finance models.
Finally, to first order, the consumption–wealth ratio is unaffected by $\overline{\alpha}_{p,t}$, a property that will be central for aggregate market elasticity.
...
Suppose $\rho>\big(1-\psi^{-1}\big)\big(\mu - \tfrac{\gamma\|\sigma\|^2}{2}\big)$.
Then:
\begin{enumerate}[label=(\roman*)]
...
Proposition~\ref{prop:first_order} shows that the main lesson from the simple model in Section~\ref{sec:simple} extends to the dynamic economy: portfolio inflows into the risky asset reduce the risk premium, while the corresponding shift out of bonds raises the risk-free rate.
To first order, these effects offset exactly, leaving the price-dividend ratio unchanged.
Hence, the aggregate market is infinitely elastic, regardless of the individual investors’ own-price elasticities $\zeta_{j,p}$.


\paragraph{Why is the market infinitely elastic?}  
As shown in Figure~\ref{fig:mkt_equilibrium}, price impact is absent as long as the consumption-wealth ratio does not respond directly to the portfolio-flow shock.
...
The term in brackets is equal to zero at the privately optimal portfolio, as implied by the investor’s first-order condition.
By the envelope theorem, small changes in $\alpha_{j,t}$ around the optimum have negligible welfare effects on welfare and hence do not affect $c_{j,t}$.    

In the first-order approximation, this derivative is evaluated at the benchmark Lucas economy, where all investors hold optimal portfolios.
The derivative is thus zero, and by Proposition~\ref{prop:first_order}, price impact is negligible for small (first-order) deviations from the frictionless allocation: the consumption-wealth ratio does not react to flow shocks.
