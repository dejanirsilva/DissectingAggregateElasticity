...
\; \alpha_{0,F,t} = 0\},
\end{equation}
where $\omega_{k,t} \equiv \frac{P_{k,t}}{P_{Y,t}}$ is the market weight of asset $k \in \{D,B\}$.
...
\]
This term captures the effect of changes in future investment opportunities on current consumption decisions.
...
Given the optimal risk exposure, the optimal portfolio shares are recovered as $\alpha_{j,t}^\top = \sigma_{j,t} \, \Sigma_t^{-1}$.
...
\paragraph{Market clearing conditions.} From the market clearing conditions, we obtain that the investors' wealth add up to the price of the aggregate claim:
\begin{equation}
...
The market clearing condition for goods and risky assets are given by:
\begin{equation}
...
These conditions echoes the one for the simple model. 
The wealth-weighted consumption-wealth ratio equals the dividend yield in the aggregate claim, and the average risk exposure equals the risk on the assets in positive net supply.
...
Since the expected return is $r_t + \pi_{k,t} = \frac{1}{p_{k,t}} + \mu_{P_k,t}$, the price-dividend ratios satisfy
\begin{equation}\label{eq:pricing_cond}
    \frac{1}{p_{k,t}} = r_t + \pi_{k,t} - (\mu + \mu_{p_k,t} + \sigma \sigma_{p_k,t}^\top),
\end{equation}
