...
\end{equation}
$c_{j}^{*} = (p_Y^{*})^{-1}$, and $\alpha_{j}^{*} = 1$ for $j = 0, 1, 2$.
\end{lemma}
...
Three results determine $\alpha_{j,1}(X)$.
First, the volatility of the valuation ratio and consumption-wealth ratios are second order:
\[
...
\]
since $p_{Y,\hat{X}}(\hat{X})$, $c_{j,\hat{X}}(\hat{X})$, $\mu_{\hat{X}}$ and $\sigma_{\hat{X}}$ are first order in $\epsilon$, as shown in Appendix~\ref{app:derivations-perturbation-solution}.
Second, using the fact that hedging demands and endogenous volatility are second order, expanding Equation~\eqref{eq:market_price_of_risk} gives the first-order correction to the market price of risk:
\begin{equation}
    \eta(x, \hat{\alpha}_p) = \gamma \sigma  \frac{1 -x_0\overline{\alpha}_p}{1-x_0} + \mathcal{O}(\epsilon^2).
\end{equation}


Third, expanding the pricing condition \eqref{eq:pricing_cond} gives
\begin{equation}
