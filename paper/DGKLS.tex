...
\begin{equation}\label{eq:D_vol}
    ...
    \mu_D(s) = \mu + \kappa_s\!\left(\frac{\overline{s}}{s}-1\right) + (1-s)\,\sigma\sigma_s^\top.
\end{equation}
...
\end{equation}
where $\lambda_t \in \mathbb{R}^{1\times d}$ is the market price of risk vector. Risk premia satisfy $\pi_{k,t} = \sigma_{R_k,t}\,\lambda_t^\top$ for each claim $k \in \{D,B,F\}$.
...
\begin{equation}
    ...
    \end{pmatrix},
\end{equation}
and $\pi_t \equiv [\pi_{D,t}, \pi_{B,t}, \pi_{F,t}]^\top$, so that $\pi_t = \Sigma_t\,\lambda_t^\top$.
...
\begin{equation}\label{eq:pricing_cond}
    \frac{1}{p_t} = r_t + \pi_t - (\mu + \mu_{p,t} + \sigma \sigma_{p,t}^\top),
\end{equation}
...
\end{equation}
with $p_{k,X} \equiv \frac{\partial p_k}{\partial X}$ and $p_{k,s} \equiv \frac{\partial p_k}{\partial s}$. Substituting into the pricing condition $\mu_{R_k,t} - r_t = \sigma_{R_k,t}\,\lambda_t^\top$ yields the partial differential equation for the valuation ratio $p_k(X,s)$:
...
&+ p_k(X,s)\,\sigma\,\sigma_{p_k}(X,s)^\top - p_k(X,s)\,\sigma\,\lambda(X)^\top - p_k(X,s)\,\sigma_{p_k}(X,s)\,\lambda(X)^\top.
\end{align}
