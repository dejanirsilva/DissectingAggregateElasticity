...
% Throughout, a superscript $\star$ denotes the benchmark economy.
...
% We proceed in two steps: first, we characterize the first-order corrections $p_1(X)$ and $c_{j,1}(X)$; second, we solve for the second-order terms $p_2(X)$ and $c_{j,2}(X)$. The zeroth-order terms $p_0(X)$ and $c_{j,0}(X)$ are given by Lemma~\ref{lemma:benchmark}, that is, $p_0(X) = p^\star$ and $c_{j,0}(X) = c_j^\star$.
...
% \(\hat\pi_t = -\,\hat r_t - p^{\star\, -1}\hat p_t + \mathcal{O}(\epsilon^2)\), we obtain
% \begin{equation}
%     \hat c_{j,t} \;\equiv\; c_{j,t} - c_{j}^\star
%     \;=\; \chi_{j,0} \;+\; \chi_{j,p}\,\hat p_t \;+\; \mathcal{O}(\epsilon^2),
% \end{equation}
% with
% $    \chi_{j,p} \;\equiv\; (\psi-1)\,\frac{1}{p^\star},$ 
%     and
...
%     \end{equation}
%     where $x_{a,t}\equiv 1-x_{0,t}$ is the wealth share of active investors and $y^\star\equiv 1/p^\star$.
% \end{prop}
...
% 	\begin{equation}\label{eq:elasticity_pref_heterog}
% 	%\frac{1}{p(X, \epsilon)} \frac{\partial p(X, \epsilon)}{\partial F(X, \epsilon)}
% 	% \varepsilon_M^{-1}=\left(1-\psi^{-1}\right) \frac{\gamma \sigma^{2}}{y^\star}\left[\frac{1-\overline{\alpha}_{p}}{x_{a}}-\frac{\gamma_{0}-\mathbb{E}^{u}[\gamma_{j}]}{\gamma}\right]+\mathcal{O}\left(\epsilon^{2}\right),
%     \varepsilon_{M,t}^{-1}=\left(1-\psi^{-1}\right) \frac{\gamma \sigma^{2}}{y^\star}\frac{\overline{\alpha}_{p,t}^{optimal}-\overline{\alpha}_{p,t}}{x_{a,t}}(1-x_{c,t})+\mathcal{O}\left(\epsilon^{2}\right).
% 	\end{equation}
