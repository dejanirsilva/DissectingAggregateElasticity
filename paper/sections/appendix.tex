...
\subsection{Special case: no preference heterogeneity or leverage constraints}
...
\paragraph{Firm's problem.} In the version of the model, we assume that the production function is given by
\begin{equation}
    \tilde{Y}_t = A_t L_t^{1-\alpha},
\end{equation}
where $A_t$ is the productivity level and $L_t$ is the labor input.
...
\begin{equation}
    \max_{L_t} = A_t L_t^{1-\alpha} - \mathcal{W}_t L_t,
\end{equation}
where $\mathcal{W}_t$ is the real wage rate.

The first-order condition gives the labor demand:
\begin{equation}
(1-\alpha) A_t L_t^{-\alpha} = \mathcal{W}_t.
\end{equation}
