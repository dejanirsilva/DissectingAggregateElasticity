...
Initially, we also assume that the portfolio share of passive investors is fixed, so $\kappa_p = 0$ and $\sigma_p = 0$.
...
\begin{equation}
    \sigma_{0,t} = \sigma + \hat{\alpha}_p \epsilon + \sigma_{p_Y,2} \epsilon^2 + \mathcal{O}(\epsilon^3).
\end{equation}
...
The first-order correction to the dividend yield is given by
\begin{equation}
    y_{Y,1} = r_1 + \eta_1 \sigma^\top
\end{equation}

The second-order correction to the dividend yield is given by
\begin{equation}
    y_{Y,2} = r_2 + \eta_2 \sigma^\top - \eta_0 \sigma_{y_Y,2}^\top + \mu_{y_Y,2} + \sigma \sigma_{y_Y,2}^\top.
\end{equation}

Combining the expressions above, we obtain
\begin{equation}
    q_{j,t} = \frac{y_{Y,1} - r_{1}}{\gamma \|\sigma\|^2} \epsilon + \left[\frac{y_{Y,2} - r_{2}- \mu_{y_Y,2} + (\gamma-1)\sigma \sigma_{y_Y,2}^\top}{\gamma \|\sigma\|^2} +\frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2} \sigma^\top}{\|\sigma\|^2}\right] \epsilon^2 + \mathcal{O}(\epsilon^3).
\end{equation}

In the absence of preference heterogeneity, we have $y_{Y,1} = p_{Y,1} = 0$ and $p_{Y,2} = - p_{Y,0}^2 y_{Y,2}$.
We can then write the demand for risky assets for active investors as follows:
\begin{equation}
    q_{j,t} = -\zeta_{j,p} \hat{p}_{Y,t} - \zeta_{j,r} \hat{r}_t + f_{j,t} + \mathcal{O}(\epsilon^3),
\end{equation}
where
\begin{equation}
    f_{j,t} =\left[- \frac{\mu_{y_Y,2} + (1-\gamma)\sigma \sigma_{y_Y,2}^\top}{\gamma \|\sigma\|^2} + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2} \sigma^\top}{\|\sigma\|^2}\right] \epsilon^2.
\end{equation}

The shifter $f_{j,t}$ is constant for active investors, and it is equal to zero when the portfolio share of passive investors is constant.
...
Using the fact that $p_{Y,2} / p_{Y,0} = - y_{Y,2} / y_{Y,0} $, we can write the shifter $f_{j,t}$ as follows:
\begin{equation}
    f_{j,t} =\left[ \frac{\mu_{p_Y,2} + (1-\gamma)\sigma \sigma_{p_Y,2}^\top}{\gamma \|\sigma\|^2} + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2} \sigma^\top}{\|\sigma\|^2}\right] \epsilon^2.
\end{equation}
