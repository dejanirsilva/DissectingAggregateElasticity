...
subject to 
\begin{equation}
    \alpha_j = \overline{\alpha}_p[ \omega_{D,t}, \omega_{B,t}, 0]^\top \qquad \text{for } j = 0, \qquad \qquad 
...
\begin{align}
    \frac{d x_{j,t}}{x_{j,t}} &= \left( r_t + \theta_t \sigma_{j,t} - \xi_{j,t} -\mu + \mu_{y,t} + \sigma \sigma_{R,t}^\top  - \sigma_{j,t} \|\sigma_{R,t}\|  +\kappa \frac{\omega_j-x_{j,t}}{x_{j,t}}  \right) dt \nonumber\\
...
\begin{equation}
    \theta_{t} =  \frac{\gamma_{u,t}}{x_{u,t}} \left[ (1-\overline{\alpha}_{p,t} x_{0,t})\|\sigma_{R_t}\| - \overline{\sigma} x_{c,t}+ \sum_{j\in \mathcal{J}_t^u} x_{j,t} \frac{1-\gamma_j^{-1}}{1-\psi} \frac{\sigma_{\xi_j,t} \sigma_{R,t}^\top}{\|\sigma_{R,t}\|}\right],
\end{equation}
...
\begin{align}
    y_t &= \psi \rho + (1-\psi) \left[ r_t + \theta_t \|\sigma_{R,t}\| - \sum_{j=0}^J x_{j,t}\frac{\gamma_j}{2} \sigma_{j,t}^2 \right] \nonumber\\
...
\begin{align}
     r_t   &  =  \rho - \theta_t \|\sigma_{R,t}\| +\psi^{-1} (\mu - \mu_{y,t}+\|\sigma_{R,t}\|^2- \sigma \sigma_{R,t}^\top)+ \left(1-\psi^{-1}\right)  \sum_{j=0}^J x_{j,t}\frac{\gamma_j}{2} \sigma_{j,t}^2 \nonumber\\
...
\begin{align}
    \xi_{j,t} &= \psi \rho + (1-\psi) \left[ r_t + \theta_t \sigma_{j,t} - \frac{\gamma_j}{2} \sigma_{j,t}^2 \right] + \frac{\xi_{j,X}}{\xi_j} \mu_{X,t} + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k,t}^\top \frac{\xi_{j,XX,t}}{\xi_{j,t}} \sigma_{X,k,t}\nonumber\\
...
Plugging the expressions for interest rate and the Sharpe ratio $(r_t, \theta_t)$, the risk exposure $\sigma_{j,t}$, the drift and diffusion of the aggregate state variables $(\mu_{X,t}, \sigma_{X,t})$, and the aggregate volatility $\|\sigma_{R,t}\|$, we can express the condition above in terms of $\xi_{j,t}$ and $y_t$ and their derivatives.

Similarly, we can write the condition for the dividend yield:
\begin{align}
    y_t &= r_t + \theta_t \left|\left|\sigma - \frac{y_{X,t}}{y_t} \sigma_{X,t}\right|\right| -\mu + \frac{y_{X,t}}{y_t} \mu_{X,t} + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k,t}^\top \frac{y_{XX,t}}{y_{t}} \sigma_{X,k,t}\nonumber\\
    &
...
Plugging in the expression for $r_0(X)$, $\theta_0(X)$ and $\sigma_{j,0}$, we obtain
\begin{equation}
...
\begin{small}
	\begin{equation}
		\sigma_{j}(X,\epsilon) = \min \left\{ \frac{\theta_0(X)}{\gamma} + \left(\frac{\theta_1(X)}{\gamma}- \frac{\theta_0(X)}{\gamma} \hat{\gamma}_j \right)\epsilon + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2}(X) \sigma^\top}{\|\sigma\|} \epsilon^2 + O(\epsilon^3)  , \|\sigma\| + \hat{\sigma} \epsilon \right\}, 
	\end{equation}
\end{small}
...
\begin{align}
    c_{j}(X,\epsilon) &= c_{j,0}(X) + (1-\psi) \left[ r_1(X) + \theta_0(X) \sigma_{j,1}(X) + \theta_{1}(X) \sigma_{j,0}(X)\right.\nonumber\\
    ...
    &= c_{j,0}(X) + (1-\psi) \gamma \|\sigma\|^2 \left[ \frac{r_1(X)}{\gamma \|\sigma\|^2}  + \frac{\theta_{1}(X)}{\gamma \|\sigma\|}  - \frac{\hat{\gamma}_j}{2}  \right]\epsilon + \mathcal{O}(\epsilon^2)\nonumber\\
