...
Let $b_j \equiv \frac{B_j}{W_j}$ denote the bond-to-wealth ratio.
...
For simplicity, focus on the case $\overline{\alpha}_p = 1$, so $b^\star_p = b_a^\star = 0$.
The passive bond demand is then given by
\begin{equation}
    x_p b_p=  f^b,
\end{equation}
where $f^b \equiv  -x_p [1-c_p(\mu-p^\star)] \hat{\alpha}_p$.
...
\begin{align}
    ...
    &\qquad + \frac{1}{2} V_{j,WW} W_j^2 \|\Sigma_j\|^2 + V_{j,WX}W_j \sigma_X \Sigma_j^\top    + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k}^\top V_{j,XX} \sigma_{X,k},
\end{align}
subject to $\alpha_j \in \Omega_j$.
...
The drift $\mu_X$ is a $N\times 1$ vector, the diffusion $\sigma_X$ is a $N\times d$ matrix, while $\sigma_R$ is a $1\times d$ vector.
...
\begin{equation}
\alpha_j = \min \left\{ -\frac{V_{j,W} (\mu_R-r)}{V_{j,WW} W \|\sigma_R\|^2}   - \frac{V_{WX}}{V_{WW} W} \frac{\sigma_X \sigma_R^\top}{\|\sigma_R\|^2}, \frac{\overline{\sigma}}{\|\sigma_{R,t}\|} \right\}.
\end{equation}
...
\begin{equation}
    \alpha_{j,t}
    = \min \left\{ \frac{\mu_{R,t}-r_t}{\gamma_j \|\sigma_{R,t}\|^2}  - \frac{1-\gamma_j^{-1}}{1-\psi} \frac{\sigma_{\xi_j,t} \sigma_{R,t}^\top}{\|\sigma_{R,t}\|^2}, \frac{\overline{\sigma}}{\|\sigma_{R,t}\|} \right\}.
\end{equation}

It is convenient to consider the investor's risk exposure $\sigma_{j} \equiv  \alpha_j \|\sigma_R\|$, which is then given by
\begin{equation}
    \sigma_{j,t} = \min \left\{ \frac{\eta_t}{\gamma_j}  - \frac{1-\gamma_j^{-1}}{1-\psi} \frac{\sigma_{\xi_j,t} \sigma_{R,t}^\top}{\|\sigma_{R,t}\|}, \overline{\sigma} \right\},
\end{equation}
...
\begin{align}
    0 &=  \frac{\rho   }{1-\psi^{-1}} \left[  \rho^{-1} \xi_j-1 \right] + r  + \eta \sigma_j   - \xi_j + \frac{1}{1-\psi} \left[ \frac{\xi_{j,X}}{\xi_j} \mu_X + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k}^\top \frac{\xi_{j,XX}}{\xi_j} \sigma_{X,k}\right]\nonumber\\
    &\qquad - \frac{\gamma_j}{2} \sigma_j^2  + \frac{1-\gamma_j}{1-\psi} \frac{\xi_{j,X}}{\xi_j} \sigma_X \frac{\sigma_R^\top}{\|\sigma_R\|} \sigma_j    + \frac{1}{2} \frac{\psi-\gamma_j}{(1-\psi)^2} \sum_{k=1}^d \sigma_{X,k}^\top \frac{\xi_{j,X}^\top}{\xi_j} \frac{\xi_{j,X}}{\xi_j} \sigma_{X,k}.
\end{align}

Rearranging the expression above, we obtain
\begin{small}
\begin{equation}
    \xi_{j,t} = \psi \rho + (1-\psi) \left[ r_t + \eta_t \sigma_{j,t} - \frac{\gamma_j}{2} \sigma_{j,t}^2 \right] + \mu_{\xi_j,t} + (1-\gamma_j)  \sigma_{\xi_j,t} \frac{\sigma_{R,t}^\top}{\|\sigma_{R,t}\|} \sigma_{j,t} + \frac{\psi - \gamma_j}{1-\psi} \frac{\|\sigma_{\xi_j,t} \|^2}{2},
\end{equation}
\end{small}
...
\begin{align}
    \mu_{\xi_j,t} &= \frac{\xi_{j,X}}{\xi_j} \mu_{X,t} + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k,t}^\top \frac{\xi_{j,XX,t}}{\xi_{j,t}} \sigma_{X,k,t}, \qquad \qquad 
...
\begin{equation}
    r_t + \eta_t \| \sigma_{R,t}\| = y_t + \frac{1}{dt} \frac{d(Y_t/y_t)}{(Y_t/y_t)} = y_t + \mu - \mu_{y,t} + \|\sigma_{y,t}\|^2 - \sigma \sigma_{y,t}^\top.
\end{equation}

Rearranging the expression above, we obtain
\begin{equation}\label{app:pricing_cond}
    y_t = r_t + \eta_t \|\sigma_{R,t}\| -\mu + \mu_{y,t} - \|\sigma_{R,t}\|^2+ \sigma \sigma_{R,t}^\top,
\end{equation}
...
We define the aggregate state variable as $X_t = (x_t, \overline{\alpha}_{p,t})$, where $x_t \equiv (x_{1,t}, x_{2,t}, \ldots, x_{J,t})$.
...
\begin{align}
    \frac{d x_{j,t}}{x_{j,t}} &= \left( r_t + \eta_t \sigma_{j,t} - \xi_{j,t} -\mu + \mu_{y,t} + \sigma \sigma_{R,t}^\top  - \sigma_{j,t} \|\sigma_{R,t}\|  +\kappa \frac{\omega_j-x_{j,t}}{x_{j,t}}  \right) dt \nonumber\\
    &\qquad + (\sigma_{j,t} - \|\sigma_{R,t}\|)\frac{\sigma_{R,t}}{\|\sigma_{R,t}\|} d Z_t,
\end{align}
using $\mu_{P,t} =\mu - \mu_{y,t} + \|\sigma_{R,t}\|^2 - \sigma \sigma_{R,t}^\top $.
...
Let $\mathcal{J}_t^u \subset \{1,2,\ldots, J\}$ denote the set of unconstrained active investors at period $t$, that is, the set of investors such that $\sigma_{j,t} < \overline{\sigma}$.
Let $\mathcal{J}_t^c \subset \{1,2,\ldots, J\}$ denote the set of constrained active investors, that is, the set of investors such that $\sigma_{j,t} = \overline{\sigma}$.
From the market clearing condition for the risky asset in Equation~\eqref{eq:mkt-clearing}, we obtain
\begin{equation}
    \eta_{t} =  \frac{\gamma_{u,t}}{x_{u,t}} \left[ (1-\overline{\alpha}_{p,t} x_{0,t})\|\sigma_{R_t}\| - \overline{\sigma} x_{c,t}+ \sum_{j\in \mathcal{J}_t^u} x_{j,t} \frac{1-\gamma_j^{-1}}{1-\psi} \frac{\sigma_{\xi_j,t} \sigma_{R,t}^\top}{\|\sigma_{R,t}\|}\right],
\end{equation}
...
\begin{align}
    ...
    &\qquad +\sum_{j=0}^J x_{j,t} \left[\mu_{\xi_j,t} + (1-\gamma_j)  \sigma_{\xi_j,t} \frac{\sigma_{R,t}^\top}{\|\sigma_{R,t}\|} \sigma_{j,t} + \frac{\psi - \gamma_j}{1-\psi} \frac{\|\sigma_{\xi_j,t} \|^2}{2}\right].
\end{align}

Using the pricing condition \eqref{app:pricing_cond}, we obtain the expression for the risk-free rate
\begin{align}
     r_t   &  =  \rho - \eta_t \|\sigma_{R,t}\| +\psi^{-1} (\mu - \mu_{y,t}+\|\sigma_{R,t}\|^2- \sigma \sigma_{R,t}^\top)+ \left(1-\psi^{-1}\right)  \sum_{j=0}^J x_{j,t}\frac{\gamma_j}{2} \sigma_{j,t}^2 \nonumber\\
    &\qquad +\psi^{-1}\sum_{j=0}^J x_{j,t} \left[\mu_{\xi_j,t} + (1-\gamma_j)  \sigma_{\xi_j,t} \frac{\sigma_{R,t}^\top}{\|\sigma_{R,t}\|} \sigma_{j,t} + \frac{\psi - \gamma_j}{1-\psi} \frac{\|\sigma_{\xi_j,t} \|^2}{2}\right].
\end{align}
...
for the $J+1$ type of investors and the dividend yield $y(X)$.
%$y(x,\overline{\alpha}_p)$.
...
\begin{align}
    \xi_{j,t} &= \psi \rho + (1-\psi) \left[ r_t + \eta_t \sigma_{j,t} - \frac{\gamma_j}{2} \sigma_{j,t}^2 \right] + \frac{\xi_{j,X}}{\xi_j} \mu_{X,t} + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k,t}^\top \frac{\xi_{j,XX,t}}{\xi_{j,t}} \sigma_{X,k,t}\nonumber\\
    &+ (1-\gamma_j)  \frac{\xi_{j,X}}{\xi_j} \sigma_{X,t} \frac{\sigma_{R,t}^\top}{\|\sigma_{R,t}\|} \sigma_{j,t} + \frac{\psi - \gamma_j}{1-\psi} \frac{1}{2} \left|\left| \frac{\xi_{j,X}}{\xi_j} \sigma_{X,t}\right|\right|^2.
\end{align}
...
\begin{align}
    y_t &= r_t + \eta_t \left|\left|\sigma - \frac{y_{X,t}}{y_t} \sigma_{X,t}\right|\right| -\mu + \frac{y_{X,t}}{y_t} \mu_{X,t} + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k,t}^\top \frac{y_{XX,t}}{y_{t}} \sigma_{X,k,t}\nonumber\\
    &
    - \left|\left|\sigma - \frac{y_{X,t}}{y_t} \sigma_{X,t}\right|\right|^2+ \sigma \left(\sigma - \frac{y_{X,t}}{y_t} \sigma_{X,t} \right)^\top,
\end{align}
...
We guess and verify that in this benchmark economy, there are no variation in expected returns.
In particular, the wealth distribution plays no role in the economy.
This implies that $\mu_{c_j,0}(X) = \sigma_{c_j,0}(X) = \mu_{p,0}(X) = \sigma_{p,0}(X) = 0$.
In this case, the risk premium is given by
\begin{equation}
...
\end{align}
where $\mu_{X,j,0} = \sigma_{X,j,0} = 0$, using the expression for returns, portfolio share, and consumption-wealth ratio.
The result  $\mu_{X,j,0} = 0$ uses the fact that $\kappa = 0$.
\end{proof}
...
\begin{proof} We consider next the first-order correction terms.
    Note that the diffusion terms for $c_j$ and $p$ are both equal to zero up to the first order, since $\sigma_{c_j,t} = \mathcal{O}(\epsilon^2)$ and $\sigma_{p,t} = \mathcal{O}(\epsilon^2)$.
    From Ito's lemma:
\begin{equation}
...
\end{equation}
We have $c_{j,X} = \mathcal{O}(\epsilon)$, because $c_{j,X,0} = 0$, as $c_{j,0}(X)$ does not depend on $X$.
Also, $\sigma_{X,t} = \mathcal{O}(\epsilon)$ because $\sigma_{X,0}(X) = 0$.
This implies that $ \sigma_{c_j,1}(X)=0 $.
An analogous argument applies to $p_t$, so that we have $\sigma_{p,1}(X) = 0$ and $\|\sigma_{R,1}(X)\| = 0$.

\paragraph{Risk exposure of active investors.} The risk exposure for active investors can be written as
\begin{equation}
    \sigma_{j}(X,\epsilon) = \min \left\{ \frac{\eta(X,\epsilon)}{\gamma_j}  + \frac{1-\gamma_j^{-1}}{\psi-1} \frac{\sigma_{c_j}(X,\epsilon) \sigma_{R}^\top(X,\epsilon)}{\|\sigma_{R}(X,\epsilon)\|}, \|\sigma\| + \hat{\sigma} \epsilon \right\},
\end{equation}
...
\begin{small}
	\begin{equation}
		\sigma_{j}(X,\epsilon) = \min \left\{ \frac{\eta_0(X)}{\gamma} + \left(\frac{\eta_1(X)}{\gamma}- \frac{\eta_0(X)}{\gamma} \hat{\gamma}_j \right)\epsilon + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2}(X) \sigma^\top}{\|\sigma\|} \epsilon^2 + O(\epsilon^3)  , \|\sigma\| + \hat{\sigma} \epsilon \right\}, 
	\end{equation}
\end{small}
...
\begin{small}
	\begin{equation}\label{eq:sigma-j}
		\sigma_{j}(X,\epsilon) = \min \left\{ \left(\frac{\eta_1(X)}{\gamma}- \frac{\eta_0(X)}{\gamma} \hat{\gamma}_j -\hat{\sigma}\right)\epsilon + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2}(X) \sigma^\top}{\|\sigma\|} \epsilon^2 + O(\epsilon^3)  , 0 \right\}+\|\sigma\| + \hat{\sigma} \epsilon.
	\end{equation}
\end{small}
...
\begin{equation}
    \left(\frac{\eta_1(X)}{\gamma}- \frac{\eta_0(X)}{\gamma} \hat{\gamma}_j -\hat{\sigma}\right)\epsilon \gg \left|\frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2}(X) \sigma^\top}{\|\sigma\|} \right| \epsilon^2,
\end{equation}
for small $\epsilon$.
...
\begin{small}
	\begin{equation}
		\sigma_{j}(X,\epsilon) = \|\sigma\| + \hat{\sigma}\epsilon+\min\left\{ \left(\frac{\eta_1(X)}{\gamma}- \frac{\eta_0(X)}{\gamma} \hat{\gamma}_j-\hat{\sigma}\right) \epsilon + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2}(X) \sigma^\top}{\|\sigma\|} \epsilon^2,0\right\} + \mathcal{O}(\epsilon^3).
	\end{equation}
\end{small}
...
The term in the square brackets in Equation~\eqref{eq:r-first-order} captures the first-order effect of frictions on the interest rate, $ r_1(X) $.
First, we see that the interest rate is decreasing in the difference between the average risk aversion of unconstrained investors and the average risk aversion of all investors in the economy.
These two averages can differ for two reasons.
First, passive investors may have a risk aversion different from the average unconstrained investor, the first term in parentheses in $ r_1(X) $.
Second, constrained investors are exactly the ones with low risk aversion, so unconstrained investors are on average more risk averse than all investors in the economy, which include the low risk aversion ones.
...
\begin{align}
    \sigma_{j,2}(X) &= \frac{\eta_2(X)}{\gamma} - \frac{\eta_1(X)}{\gamma}\hat{\gamma}_j + \frac{\eta_0(X)}{\gamma}\hat{\gamma}_j^2 + \frac{1-\gamma^{-1}}{\psi-1} \frac{\sigma_{c_j,2}(X) \sigma^\top}{\|\sigma\|},
\end{align}
...
\begin{small}
	\begin{equation}
	\eta(X,\epsilon) =  \frac{\gamma_u(X,\epsilon)}{x_{u}} \left[ (1-(1+\hat{\alpha}_p \epsilon) x_{0,t})\|\sigma - \sigma_{y}(X,\epsilon)\| - \|\sigma\|(1 + \hat{\sigma}\epsilon) x_{c,t}+ \sum_{j\in \mathcal{J}_t^u} x_{j,t} \frac{1-\gamma_j^{-1}}{1-\psi} \sigma_{c_j}(X,\epsilon) \frac{\sigma^\top}{\|\sigma\|}\right],
	\end{equation}	
\end{small}

The second-order term is then given by
\begin{align}
    \eta_2(X) &=  \frac{\gamma_{u,0}(X)}{x_{u}} \left[ -(1-x_{0,t})\sigma_{y,2}(X)\frac{\sigma^\top}{\|\sigma\|} + \sum_{j\in \mathcal{J}_t^u} x_{j,t} \frac{1-\gamma^{-1}}{1-\psi} \sigma_{c_j,2}(X) \frac{\sigma^\top}{\|\sigma\|}\right] + \nonumber\\
...
\begin{align}
    \eta_2(X) =  - \frac{(\gamma-1) x_c + 1-x_0}{1-x_0-x_c} \sigma_{y,2}(X) \frac{\sigma^\top}{\|\sigma\|} - \gamma \|\sigma\| \mathbb{E}^u[\hat{\gamma}_j] \frac{ \hat{\alpha}_p x_0+ \hat{\sigma} x_c }{1-x_0-x_c} - \gamma \|\sigma\| \var^u[\hat{\gamma}_j].
\end{align}
...
The interest rate is given by
\begin{align}
     r(X,\epsilon)   &  =  \rho - \eta(X,\epsilon) \|\sigma_{R}(X,\epsilon)\| +\psi^{-1} (\mu - \mu_{y}(X,\epsilon)- \sigma_y \sigma_{R}(X,\epsilon)^\top)\nonumber\\
...
\begin{align}
    r_2(X) &= - \eta_2(X) \|\sigma\| + \eta_0(X) \sigma_{y,2}(X) \frac{\sigma^\top}{\|\sigma\|}-\psi^{-1} (\mu_{y,2}(X) + \sigma_{y,2}(X)\sigma^\top)\nonumber\\
...
\begin{small}
	\begin{align}
	y_2(X) &=  (1-\psi) \left[ r_2(X) + \eta_2(X) \|\sigma\| + \eta_0(X) \sigma_{R,2}(X) \frac{\sigma^\top}{\|\sigma\|} - \sum_{j=0}^J x_{j}\frac{\gamma}{2} \left( \sigma_{j,1}^2(X) + 2 \sigma_{j,0} \sigma_{j,2}(X) + 2 \hat{\gamma}_j \sigma_{j,0} \sigma_{j,1}(X)  \right)\right] \nonumber\\
...
\left(-\eta_{0}(X) \sigma_{y,2}(X) \frac{\sigma^\top}{\|\sigma\|} +\eta_2(X)\|\sigma\|\right)\epsilon^2+\mathcal{O}\left(\epsilon^3\right).
\end{equation*}
We can then write
\begin{equation}\label{eq:rp-second-order}
	\pi_2(X) = -\eta_{0}(X)\sigma_{y, 2}(X) \frac{\sigma^\top}{\|\sigma\|}+\eta_2(X)\|\sigma\|.
\end{equation}
...
Fourth, conditional on $(\hat y,\pi_Y,\pi_F,\sigma_y)$, the risk-free rate $r(X)$, the state drift $\mu_X(X)$, the state diffusion matrix $\Sigma_X(X)$, the consumption-wealth ratio drifts $\mu_{c_j}(X)$, and the market price of risk vector $\lambda(X)$ are all recovered from the equilibrium identities and It\^o's lemma.
...
For each state $(X,s)$, the first-stage solution supplies the coefficients entering the claim-pricing PDEs: the risk-free rate $r(X)$, the market price of risk $\lambda(X)$, the state drift $\mu_X(X)$, the state diffusion matrix $\Sigma_X(X)$, and the aggregate-claim return volatility.
