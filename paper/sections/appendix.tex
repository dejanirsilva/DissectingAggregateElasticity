...
\end{equation}
Substituting \eqref{eq:delta-nu22-alpha-p} into \eqref{eq:delta-eta2-system-alpha-p} gives the linear system
...
\subsection{The case of no preference heterogeneity or leverage constraints}

Consider the case of no preference heterogeneity or leverage constraints, that is, $\hat{\gamma}_j=0$ for $j=0,1,2$ and $\hat{\nu}_2=0$.

\begin{prop}
    The time-varying-$\alpha_p$ contribution to the second-order dividend yield, risk premium, and interest rate is given by
    \begin{equation}
        \Delta y_{Y,2}
        =-\frac{G_p}{y_{Y,0}}\sigma_p\sigma^\top,
    \end{equation}
    \begin{equation}
        \Delta\pi_{Y,2}
        =-\frac{\gamma G_p}{y_{Y,0}}\sigma_p\sigma^\top.
    \end{equation}
    \begin{equation}
        \Delta r_2
        =\gamma\frac{\psi-\gamma^{-1}}{\psi-1}
...
\begin{equation}
    y_{Y,2}^{0}(x,\hat{\alpha}_p)=d_Y^0\hat{\alpha}_p^2,
    \qquad
    d_Y^0=\frac{1}{2}(1-\psi^{-1})\gamma\|\sigma\|^2\frac{x_0}{x_a}.
\end{equation}
Thus, $a_Y^0=b_Y^0=0$.
With time-varying passive demand, the total second-order dividend-yield correction is
\begin{equation}
...
\end{equation}
Using the coefficient formulas above,
\begin{align}
    d_Y^T
    &=\frac{1}{1+\frac{2\kappa_p}{y_{Y,0}}}d_Y^0,\\
    b_Y^T
...
\end{equation}
Equivalently,
\begin{equation}\label{eq:yY2-total-noheterog}
    y_{Y,2}^{T}(x,\hat{\alpha}_p)
    =\frac{1}{y_{Y,0}}\left(b_Y^T M_p+d_Y^T V_p\right)
    +\frac{2d_Y^T M_p}{y_{Y,0}+\kappa_p}\hat{\alpha}_p
    +d_Y^T\hat{\alpha}_p^2.
\end{equation}
It is useful to define the slope of the total second-order dividend yield with respect to the passive portfolio share,
\begin{equation}\label{eq:G-alpha-noheterog}
    G_p(x,\hat{\alpha}_p)
    \equiv \frac{\partial y_{Y,2}^{T}}{\partial \hat{\alpha}_p}
    = b_Y^T+2d_Y^T\hat{\alpha}_p
    =2d_Y^T\left(\hat{\alpha}_p+\frac{M_p}{y_{Y,0}+\kappa_p}\right).
\end{equation}
Therefore, the additional drift and diffusion of the dividend yield generated by time-varying passive demand are
\begin{align}
    \Delta\sigma_{y_Y,2}
    &=\frac{G_p}{y_{Y,0}}\sigma_p,\\
    \Delta\mu_{y_Y,2}
    &=\frac{G_p}{y_{Y,0}}\kappa_p(\hat{\alpha}-\hat{\alpha}_p)
    +\frac{d_Y^T}{y_{Y,0}}\sigma_p\sigma_p^\top.
\end{align}
The second expression uses $(y_{Y,2}^{T})_{\hat{\alpha}_p\hat{\alpha}_p}=2d_Y^T$.
...
\begin{equation}
    \pi_{Y,t}=\sigma_{R_Y,t}\eta_t^\top.
\end{equation}
Since $\sigma_{R_Y,t}=\sigma-\sigma_{y_Y,2}\epsilon^2+\mathcal{O}(\epsilon^3)$, the time-varying-$\alpha_p$ contribution to the second-order risk premium is
\begin{equation}\label{eq:delta-piY2-noheterog-start}
    \Delta\pi_{Y,2}
    =-\Delta\sigma_{y_Y,2}\eta_0^\top+\sigma\Delta\eta_2^\top.
\end{equation}
In the no-heterogeneity and no-constraint case, $c_{j,2}^{T}=y_{Y,2}^{T}$ for all investors, so
\begin{equation}
    \Delta\sigma_{c_j,2}=\Delta\sigma_{y_Y,2}=\frac{G_p}{y_{Y,0}}\sigma_p.
\end{equation}
The market price of risk correction from Step 3 therefore becomes
\begin{equation}
    \Delta\eta_2
    =-\gamma\left[1+\frac{1-\gamma^{-1}}{\psi-1}\right]\Delta\sigma_{y_Y,2}.
\end{equation}
Using
\begin{equation}
    1+\frac{1-\gamma^{-1}}{\psi-1}=\frac{\psi-\gamma^{-1}}{\psi-1},
\end{equation}
we obtain
\begin{equation}\label{eq:delta-eta2-noheterog}
    \Delta\eta_2
    =-\gamma\frac{\psi-\gamma^{-1}}{\psi-1}\frac{G_p}{y_{Y,0}}\sigma_p.
\end{equation}
Substituting the expression for $\Delta\sigma_{y_Y,2}$ above and \eqref{eq:delta-eta2-noheterog} into \eqref{eq:delta-piY2-noheterog-start}, and using $\eta_0=\gamma\sigma$, gives
\begin{align}\label{eq:delta-piY2-noheterog}
    \Delta\pi_{Y,2}
    &=-\gamma\frac{G_p}{y_{Y,0}}\sigma_p\sigma^\top
    -\gamma\frac{\psi-\gamma^{-1}}{\psi-1}\frac{G_p}{y_{Y,0}}\sigma\sigma_p^\top \\
    &=-\frac{\gamma G_p}{y_{Y,0}}
    \left[\sigma_p\sigma^\top+\frac{\psi-\gamma^{-1}}{\psi-1}\sigma\sigma_p^\top\right].
\end{align}
Thus, the time-varying passive-share contribution to the risk premium is proportional to the slope $G_p$ of the total second-order dividend yield with respect to the passive portfolio share.

\paragraph{Interest rate.}
The interest-rate correction follows directly from the simplification in Step 5.
The incremental contribution of time-varying passive demand is
\begin{equation}
    \Delta r_2=-\sigma\Delta\eta_2^\top.
\end{equation}
Using \eqref{eq:delta-eta2-noheterog}, we obtain
\begin{equation}\label{eq:delta-r2-noheterog}
    \Delta r_2
    =\gamma\frac{\psi-\gamma^{-1}}{\psi-1}
    \frac{G_p}{y_{Y,0}}\sigma\sigma_p^\top.
\end{equation}
Thus, the effect of time-varying passive demand on the interest rate is governed by the same slope $G_p$ that determines the diffusion of the dividend yield and the risk premium.

The total second-order interest-rate correction is
\begin{equation}
    r_2^T=r_2^0+\Delta r_2,
\end{equation}
where $r_2^0$ denotes the fixed-$\hat{\alpha}_p$ correction.
Equivalently, using the pricing identity
\begin{equation}
    y_{Y,2}^T=r_2^T+\pi_{Y,2}^T+\mu_{y_Y,2}^T+\sigma\sigma_{y_Y,2}^{T\top},
\end{equation}
we can write
\begin{equation}\label{eq:r2-total-noheterog}
    r_2^T
    =y_{Y,2}^T-\pi_{Y,2}^T-
    \mu_{y_Y,2}^T-\sigma\sigma_{y_Y,2}^{T\top}.
\end{equation}
In the no-heterogeneity case, the time-varying component of this expression reduces to \eqref{eq:delta-r2-noheterog} because the changes in the dividend-yield drift and diffusion exactly offset the corresponding terms in the pricing identity.
