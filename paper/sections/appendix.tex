...
\section{The first-order demand system}


Hedging demands and endogenous volatility are second order in $\epsilon$:
\begin{align}
    \sigma_{c_j} &= \mathcal{O}(\epsilon^2), \qquad \qquad 
    \sigma_{p_Y} = \mathcal{O}(\epsilon^2).
\end{align}

The risk premium on the aggregate claim is given by
\begin{equation}
    \pi_Y = \sigma_{R_Y} \eta^\top =  + \mathcal{O}(\epsilon^2),
\end{equation}
using the fact that $\sigma_{R_Y} = \sigma + \mathcal{O}(\epsilon^2)$.

In the absence of endogenous volatility, return volatility is driven only by cash-flow news. 
Hence, investors have no incentive to trade the derivative, so $\alpha_{j,F} = 0$ for all $j$.
The exposure to the aggregate claim satisfies the condition:
\begin{equation}
    \sigma_{j} = \alpha_{j,Y} \sigma + \mathcal{O}(\epsilon^2) \Rightarrow \sigma_{j} = \alpha_{j,Y} \sigma + \mathcal{O}(\epsilon^2) \Rightarrow \alpha_{j,Y} = \frac{\sigma_{j} \sigma^\top}{\sigma \sigma^\top} + \mathcal{O}(\epsilon^2),
\end{equation}
so $\sigma_{R,Y} = \sigma + \mathcal{O}(\epsilon^2)$ and $h_a = 0 + \mathcal{O}(\epsilon^2)$.

Hence, the market price of risk can be written as
\begin{equation}
...
The risk premium on the aggregate claim is given by
\begin{equation}
    \pi_Y = \sigma_{R_Y} \eta^\top = \gamma \|\sigma\|^2  \frac{1 -x_0\overline{\alpha}_p}{1-x_0} + \mathcal{O}(\epsilon^2),
\end{equation}
using the fact that $\sigma_{R_Y} = \sigma + \mathcal{O}(\epsilon^2)$.

Hence, the return volatility of the aggregate claim is given by
\begin{equation}
    \sigma_{R_Y} = \sigma + \mathcal{O}(\epsilon^2).
\end{equation}

Using the fact that hedging demands and endogenous volatility are second order, expanding Equation~\eqref{eq:market_price_of_risk} gives the first-order correction to the market price of risk:
\begin{equation}
    \eta(x, \hat{\alpha}_p) = \gamma \sigma  \frac{1 -x_0\overline{\alpha}_p}{1-x_0} + \mathcal{O}(\epsilon^2).
\end{equation}
