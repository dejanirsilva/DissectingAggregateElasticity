...
We will show that one can solve for a reduced system of PDEs involving $(c_j,p_Y)$, so the split between dividends and bond payments does not affect the equilibrium.
...
Consider the following block describing the risk exposure of investor $j = 1,2$:
\begin{align}
    \sigma_{j,t} &= \frac{1}{1+\nu_{j,t}} \left[ \frac{\eta_t}{\gamma_j} + \frac{1-\gamma_j^{-1}}{\psi-1} \, \sigma_{c_j,t} \right],\\
    \eta_t &= \gamma_{a,t} \left[\chi_{a,t}\sigma_{R_Y,t} - \sum_{j=0}^2 x_{j,t} \frac{1-\gamma_j^{-1}}{\psi-1} \, \frac{\sigma_{c_j,t}}{1+\nu_{j,t}}\right],\\
    \sigma_{R_Y,t} &= \sigma + \sum_{k=1}^2 \frac{p_{Y,x_k,t}}{p_{Y,t}} \sigma_{x_k,t} + \frac{p_{Y,k,\overline{\alpha}_p,t}}{p_{Y,t}} \sigma_{\overline{\alpha}_p,t} + \frac{p_{Y,k,s,t}}{p_{Y,t}} \sigma_{s,t},\\
    \sigma_{c_j,t} &= \sum_{k=1}^2 \frac{c_{j,x_k,t}}{c_{j,t}} \sigma_{x_k,t} + \frac{c_{j,k,\overline{\alpha}_p,t}}{c_{j,t}} \sigma_{\overline{\alpha}_p,t} + \frac{c_{j,k,s,t}}{c_{j,t}} \sigma_{s,t},
\end{align}
where $\alpha_{j,t} = \frac{P_{j,t}}{P_{Y,t}}$ for $j \in \{D,B\}$.

We can then solve for $\sigma_{j,t}$ in terms of the risk exposure of the endogenous state variables:
\begin{equation}
    \sigma_{j,t} = \chi_{\sigma_j,0}(X_t) + \chi_{\sigma_j,1}(X_t) \sigma_{x_1,t} + \chi_{\sigma_j,2}(X_t) \sigma_{x_2,t},
\end{equation}
where $\chi_{\sigma_j,k}, k=0,1,2$ depend on $c_j$ and $p_{Y}$ and their derivatives and the aggregate state variables $X_t$.

The risk exposure of the endogenous state variables is given by
\begin{equation}
    \sigma_{x_j,t} = x_{j,t} \left[ \sigma_{j,t} -  \sum_{k=0}^2 x_{k,t }\sigma_{x_k,t}  \right].
\end{equation}

This gives us a system of equations determining $\sigma_{x_j,t}$:\small
\begin{equation}
        \begin{bmatrix}
            1 - x_{1,t} \sum_{k=0}^2 \left( \chi_{\sigma_1,1} - \chi_{\sigma_k,1} \right) x_{k,t} & - x_{1,t} \sum_{k=0}^2 \left( \chi_{\sigma_1,2} - \chi_{\sigma_k,2} \right) x_{k,t}\\
            - x_{2,t} \sum_{k=0}^2 \left( \chi_{\sigma_2,1} - \chi_{\sigma_k,1} \right) x_{k,t} & 1 - x_{2,t} \sum_{k=0}^2 \left( \chi_{\sigma_2,2} - \chi_{\sigma_k,2} \right) x_{k,t}
        \end{bmatrix}
        \begin{bmatrix}
            \sigma_{x_1,t}\\
            \sigma_{x_2,t}
        ...
        =
        \begin{bmatrix}
          x_{1,t} \sum_{k=0}^2 \left( \chi_{\sigma_1,0} - \chi_{\sigma_k,0} \right) x_{k,t} \\
          x_{2,t} \sum_{k=0}^2 \left( \chi_{\sigma_2,0} - \chi_{\sigma_k,0} \right) x_{k,t}
        \end{bmatrix}.
\end{equation}\normalsize

Solving the system above, we obtain $\sigma_{x_j,t}$ as a function of $X_{j,t}$ and the functions $c_j$ and $p_{Y}$ and their derivatives.
Given $\sigma_{x_j,t}$, we can solve for $\sigma_{j,t},$ $\eta_t$, $\sigma_{c_j,t}$, and $\sigma_{R_Y,t}$ using the expressions above. 
The risk premium on the aggregate claim is given by $\pi_{Y,t} = \sigma_{R_Y,t} \eta_t^\top$.

\paragraph{Step 2: Drift of endogenous state variables.} The drift of the endogenous state variables is given by 
\begin{align}
    \mu_{x_j,t} &= x_{j,t} \left[ 
        (\eta_t-\sigma_{R_Y,t}) \left( \sigma_{j,t} -\sigma_{R_Y,t} \right)^\top - \left( c_{j,t} - \frac{1}{p_{Y,t}}  \right)
     \right] + \kappa (\omega_j - x_{j,t}).
\end{align}

Given $\mu_{x_j,t}$, we can solve for the drift of $c_j,t$ and $p_{Y,t}$ using Ito's lemma:
\begin{align}
    \mu_{c_j,t} &= \frac{c_{j,X,t}}{c_{j,t}} \mu_{X,t} + \frac{1}{2}\sum_{k=1}^2 \sigma_{X,k,t}^\top\,\frac{c_{j,XX,t}}{c_{j,t}}\,\sigma_{X,k,t},\\
    \mu_{p_Y,t} &= \frac{p_{Y,X,t}}{p_{Y,t}} \mu_{X,t} + \frac{1}{2}\sum_{k=1}^2 \sigma_{X,k,t}^\top\,\frac{p_{Y,XX,t}}{p_{Y,t}}\,\sigma_{X,k,t}.
\end{align}
...
We can then solve for $\xi_t = \sum_{j=0}^2 x_{j,t} \xi_{j,t}$ and the interest rate:
\begin{align}
    r_t &= \rho 
    + \psi^{-1} \left(\mu_{p_Y,t} + \mu + \sigma \sigma_{p_Y,t}^\top + \xi_t \right)
    + (1-\psi^{-1}) \sum_{j=0}^2 x_{j,t} 
        \frac{\gamma_j \alpha_{j,t}^2}{2} \|\sigma_{R_Y,t}\|^2
    - \pi_{Y,t}.
\end{align}

\paragraph{Step 4: PDE for the consumption-wealth ratio and price-dividend ratio.}
\begin{align}
        c_{j,t} &= \psi \rho 
        + (1-\psi) \left[ r_t + \eta_{t}^\top\,\sigma_{j,t} - \frac{\gamma_j}{2} \|\sigma_{j,t}\|^2 \right] 
        +  \mu_{c_j,t} 
        + (1-\gamma_j) \sigma_{c_j,t} \sigma_{j,t}^\top 
        + \frac{\psi - \gamma_j}{1-\psi} \frac{\|\sigma_{c_j,t}\|^2}{2}\\
        \frac{1}{p_{Y,t}} &= r_t + \eta_{t}^\top\,\sigma_{R_Y,t} - \big(\mu + \mu_{p_Y,t} + \sigma \, \sigma_{p_Y,t}^\top\big),        
\end{align}
Notice that $(r_t, \eta_t, \sigma_{j,t}, \mu_{c_j,t}, \sigma_{c_j,t}, \mu_{p_Y,t}, \sigma_{p_Y,t})$ are all functions of $X_t$, $(c_j,p_Y)$ and their derivatives.
