...
subject to 
\begin{equation}
    \alpha_j = [\overline{\alpha}_p \omega_{D,t}, \overline{\alpha}_p \omega_{B,t}, 0]^\top \qquad \text{for } j = 0, \qquad \qquad 
    \sigma_j \sigma_j^\top \leq \overline{\sigma}^2 \qquad \text{for } j = 2.
\end{equation}
given $\sigma_{j} = \alpha_j^\top \Sigma$.
...
\begin{equation}
\sigma_j =  -\frac{V_{j,W}}{V_{j,WW} W_j + \tilde{\nu}_{j,t}}  \lambda_j   - \frac{V_{j,WX}}{V_{j,WW} W_j + \tilde{\nu}_{j,t}} \sigma_X.
\end{equation}
using the fact that $\pi = \Sigma \lambda^\top$.
...
\begin{equation}
    \sigma_{j,t}
    =\frac{1}{1+\nu_{j,t}} \left[ \frac{\lambda_t}{\gamma_j}  + \frac{1-\gamma_j^{-1}}{\psi-1} \sigma_{c_j,t} \right],
\end{equation}
where $\nu_{j,t} \equiv \tilde{\nu}_{j,t} / (V_{j,WW,t} W_{j,t})$.

Plugging the consumption-wealth ratio into the HJB equation and using Equation \eqref{app:value_function}, we obtain
\begin{align}
        0 &=  \frac{\rho   }{1-\psi^{-1}} \left[  \rho^{-1} c_j-1 \right] + r  + \lambda \sigma_j^ \top   - c_j + \frac{1}{1-\psi} \left[ \frac{\xi_{j,X}}{\xi_j} \mu_X + \frac{1}{2} \sum_{k=1}^d \sigma_{X,k}^\top \frac{\xi_{j,XX}}{\xi_j} \sigma_{X,k}\right]\nonumber\\
...
\begin{small}
\begin{equation}
    c_{j,t} = \psi \rho + (1-\psi) \left[ r_t + \lambda_t \sigma_{j,t}^\top - \frac{\gamma_j}{2} \| \sigma_{j,t}\|^2 \right] + \mu_{c_j,t} + (1-\gamma_j)  \sigma_{c_j,t} \sigma_{j,t}^\top + \frac{\psi - \gamma_j}{1-\psi} \frac{\|\sigma_{c_j,t} \|^2}{2},
\end{equation}
\end{small}
...
The pricing condition for asset $k \in\{D,B, Y\}$ is given by
\begin{equation}
    r_t + \sigma_{R_k,t} \lambda_t^\top = \frac{1}{p_{k,t}} + \mu_{k,t} + \mu_{p_k,t} + \sigma_{k,t} \sigma_{p_k,t}^\top,
\end{equation}
...
Notice that $\mu_{R_Y,t} = \omega_{D,t} \mu_{R_D,t} + \omega_{B,t} \mu_{R_B,t}$, where $\omega_{D,t} = \frac{P_{D,t}}{P_{Y,t}}$ and $\omega_{B,t} = \frac{P_{B,t}}{P_{Y,t}}$.
...
\begin{equation}
    \sum_{j=0}^2 x_{j,t} \alpha_{j,D,t} = \omega_{D,t}, \qquad 
    \sum_{j=0}^2 x_{j,t} \alpha_{j,B,t} = \omega_{B,t}, \qquad 
    \sum_{j=0}^2 x_{j,t} \alpha_{j,F,t} = 0.
\end{equation}  

Let $\omega_t = [\omega_{D,t}, \omega_{B,t},0]^\top$ denote the market weights of the dividend, bond, and derivative claims.
We can then write the market clearing conditions above in a compact way:
\begin{equation}
    \sum_{j=0}^2 x_{j,t} \alpha_{j,t} = \omega_t.
\end{equation}
...
\begin{equation}
    \frac{d W_{j,t}}{W_{j,t}} = \left[ r_t + \lambda_t \sigma_{j,t}^\top - c_{j,t} \right]dt + \sigma_{j,t} d Z_t.
\end{equation}
