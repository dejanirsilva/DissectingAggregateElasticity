\section{Derivations}
\subsection{Derivations for Section \ref{sec:simple}}
\subsubsection{The bond demand view}
...
\subsection{Derivations for Section \ref{sec:model}}

\subsubsection{Investors' problem}
...
\subsubsection{Pricing condition}
...
\subsubsection{Aggregate state variable}
...
\subsubsection{Block recursivity and the mutual fund theorem}
...
Moreover, the solution satisfies the mutual fund theorem, that is, investors hold risky claims in proportion to their market-capitalization weights.
This result is summarized in Proposition~\ref{prop:mft}.

We provide below the proof of Proposition~\ref{prop:mft} in two steps. 
First, we derive the system of PDEs and show the block-recursive structure in Lemma~\ref{lemma:block_recursivity}.
Second, we show the mutual fund theorem holds in Lemma~\ref{lemma:mutual_fund_theorem}.

\paragraph{Block recursivity.}
...
\paragraph{The mutual fund theorem}

We next show that a version of the mutual fund theorem holds in our economy.

\begin{lemma}[Mutual fund theorem]\label{lemma:mutual_fund_theorem}
    The portfolio shares of active investors are given by:
    \begin{equation}
...
\begin{equation}
    Q_{j,D,t} = \alpha_{j,D,t} \frac{W_{j,t}}{P_{D,t}} = \alpha_{j,Y,t} \frac{W_{j,t}}{P_{Y,t}},
    \qquad \qquad 
    Q_{j,B,t} = \alpha_{j,B,t} \frac{W_{j,t}}{P_{B,t}} = \alpha_{j,Y,t} \frac{W_{j,t}}{P_{Y,t}}.
\end{equation}
Hence, the holdings of the dividend claim and the bond claim are equalized for each investor: $Q_{j,D,t} = Q_{j,B,t}$.
...
\subsubsection{Closed-form solution to individual claims}
...
This corresponds to the case in the economy without preference heterogeneity and passive investors are fully invested in the risky asset, $\overline{\alpha}_{p,t} = 1$.
To obtain closed-form solutions, analogous to the ones in \citet{MenzlySantosVeronesi2004}, we focus in the special case $\gamma = 1$. 
The closed-form solutions provide a useful reference to compare with the perturbation solution derived in the next section.
...
\subsection{Derivations for Section \ref{sec:elasticity}: Perturbation Solution}\label{app:derivations-perturbation-solution}
...
\subsubsection{Proof of Lemma~\ref{lemma:benchmark}}\label{app:lemma1}
...
\subsubsection{Proof of Proposition~\ref{prop:first_order}}\label{app:prop_first_order}
...
\end{equation}
This implies $\overline{\alpha} = 1+\hat{\alpha} \epsilon$ and that the diffusion of $\overline{\alpha}_{p}$ is of order $\mathcal{O}(\epsilon)$. The main text focus on the case $\hat{\alpha} = 0$, so the unconditional value of the portfolio share is $\overline{\alpha}_{p} = 1$.
...
\begin{equation}
    \frac{x_1}{x_a}\left(\hat{\gamma}_1 - \hat{\gamma}_2 - \hat{\nu}_{2}\right) -\frac{x_0}{x_a}\hat{\alpha}_{p} \leq \hat{\sigma}.
\end{equation}
If the constraint binds, this condition holds with equality, which implies
\begin{equation}
    \hat{\nu}_{2} = \hat{\gamma}_1-
    \hat{\gamma}_2 -\frac{x_0}{x_1}\hat{\alpha}_{p} -\frac{x_a}{x_1} \hat{\sigma}.
\end{equation}
If the constraint is slack, then $\hat{\nu}_{2}=0$ and
\begin{equation}
    \frac{x_1}{x_a}\left(\hat{\gamma}_1 - \hat{\gamma}_2 \right) -\frac{x_0}{x_a}\hat{\alpha}_{p} < \hat{\sigma},
\end{equation}
or, equivalently,
\begin{equation}
    \hat{\gamma}_1-\hat{\gamma}_2 -\frac{x_0}{x_1}\hat{\alpha}_{p} -\frac{x_a}{x_1} \hat{\sigma} <0.
\end{equation}
Hence, the first-order Lagrange multiplier is
\begin{equation}
    \hat{\nu}_{2} = \max\left\{ \hat{\gamma}_1-\hat{\gamma}_2 -\frac{x_0}{x_1}\hat{\alpha}_{p} -\frac{x_a}{x_1} \hat{\sigma}, 0 \right\}.
\end{equation}

When the constraint binds, substituting the expression for $\hat{\nu}_2$ into the market price of risk yields
\begin{equation}
    \eta_1 = \gamma \sigma \left[\hat{\gamma}_1 - \frac{x_2}{x_1}\hat{\sigma}  -\frac{x_0}{x_1}\hat{\alpha}_{p}  \right].
\end{equation}
When the constraint is slack, $\hat{\nu}_2=0$, and the market price of risk is
\begin{equation}
    \eta_1  =  \gamma \sigma \left[\sum_{j=1}^2 \frac{x_j}{x_a} \hat{\gamma}_j -\frac{x_0}{x_a}\hat{\alpha}_{p} \right].
\end{equation}
...
\begin{equation}
    \varepsilon_{Y,t}^{-1} = \frac{\partial \hat{p}_{Y,t}}{\partial f_t} = \frac{1}{x_{0,t} p_{Y}^*} \frac{\partial p_{Y,1}}{\partial \hat{\alpha}_p} = 0.
\end{equation}
...
We next solve for the valuation ratios $p_{k,t} \equiv P_{k,t}/Y_t$, $k\in\{D,B\}$.
...
with
$
    \overline{s}_D=\overline{s}, \overline{s}_B=1-\overline{s}, 
    \hat{s}_D=\hat{s}, 
$ and $\hat{s}_B=-\hat{s}$.
...
\begin{equation}
    b_D = \frac{1}{\delta + \kappa_s},\qquad \qquad
...
\subsubsection{Second-order correction: no dynamics in passive portfolio share}\label{app:prop_second_order}
...
\subsubsection{Second-order correction: time-varying passive portfolio share}\label{app:prop_second_order_time_varying}
...
\section{Deriving the aggregate market elasticity}

\subsection{No preference heterogeneity or leverage constraints}
...
The next proposition summarizes the results for this special case.
It encompasses the results of Proposition~\ref{prop:elasticity_passive_demand} and Proposition~\ref{prop:market_price_risk_risk_premium_interest_rate}.
...
\begin{prop}[Time-varying passive demand without heterogeneity or constraints]
    ...
    \begin{equation}
        H_a
        \equiv
        \frac{1-\gamma^{-1}}{\psi-1} \left(\frac{\psi}{x_a} - 1\right).
    \end{equation}
    The risk premium is
    \begin{equation}
        \pi_Y
...
First, consider the case with no preference heterogeneity and no leverage constraints, that is, $\hat{\gamma}_j=0$ for $j=0,1,2$ and $\hat{\nu}_2=0$.
In this case, the dividend yield is given by
\begin{equation}    
...
\begin{align}
    (\epsilon_{Y,t}^0)^{-1} &= -\frac{1}{x_{0,t} y_{Y,0}} \frac{\partial y_{Y,2}^0}{\partial \hat{\alpha}_p} \epsilon + \mathcal{O}(\epsilon^2). \\
    &= -\frac{1}{x_{0,t} y_{Y,0}} \left(2 d_y^0 \hat{\alpha}_p + b_y^0 \right) \epsilon + \mathcal{O}(\epsilon^2).
\end{align}

In the case with preference heterogeneity and leverage constraints, the inverse aggregate market elasticity is given by
\begin{equation}
    (\epsilon_{Y,t}^0)^{-1} = (1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{y_{Y,0}} \frac{\overline{\alpha}_p^{opt}-1}{x_a-x_c} (1-x_c) +(1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{y_{Y,0}} \frac{1-\overline{\alpha}_p}{x_a-x_c} (1-x_c)  + \mathcal{O}(\epsilon^2).
\end{equation}

This implies that the coefficients are given by
\begin{align}   
    \frac{2 d_Y^0}{x_{0,t} y_{Y,0}} &= (1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{y_{Y,0}} \frac{(1-x_c)}{x_a-x_c} , \\
    -\frac{b_Y^0}{x_{0,t} y_{Y,0}} \epsilon &= \frac{2 d_Y^0}{x_{0,t} y_{Y,0}} (\overline{\alpha}_p^{opt}-1).
\end{align}
...
where
\begin{align}
    d_{Y}^T &= \frac{y_{Y,0}}{y_{Y,0}+2\kappa_p} d_Y^0,\\
    b_{Y}^T &= \frac{y_{Y,0}}{y_{Y,0}+\kappa_p} b_Y^0 + \frac{2 d_Y^T}{y_{Y,0}+\kappa_p} M_p,
\end{align}
where $M_p = \kappa_p \hat{\alpha} + (1-\gamma) \sigma_p \sigma^\top$.

The inverse aggregate market elasticity is given by
\begin{align}
    (\epsilon_{Y,t}^T)^{-1} 
    &= -\frac{1}{x_{0,t} y_{Y,0}} \left(2 d_Y^T \hat{\alpha}_p + b_Y^T \right) \epsilon + \mathcal{O}(\epsilon^2).
\end{align}

The coefficient on $1-\overline{\alpha}_p$ can be written as
\begin{equation}
    \frac{2 d_Y^T}{x_{0,t} y_{Y,0}} = \frac{y_{Y,0}}{y_{Y,0}+2\kappa_p} \frac{2 d_Y^0}{x_{0,t} y_{Y,0}} = (1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{y_{Y,0}+2\kappa_p} \frac{(1-x_c)}{x_a-x_c} ,
\end{equation}
and the intercept term is given by
\begin{align}
    -\frac{b_Y^T}{x_{0,t} y_{Y,0}} &= - \frac{y_{Y,0}}{y_{Y,0}+\kappa_p} \frac{b_Y^0}{x_{0,t} y_{Y,0}} - \frac{M_p}{y_{Y,0}+\kappa_p} \frac{2 d_Y^T}{x_{0,t} y_{Y,0}}\\
    &= \frac{y_{Y,0}}{y_{Y,0}+\kappa_p} \frac{2 d_Y^0}{x_{0,t} y_{Y,0}} \hat{\alpha}_p^{opt} - \frac{M_p}{y_{Y,0}+\kappa_p} \frac{y_{Y,0}}{y_{Y,0}+2\kappa_p} \frac{2 d_Y^0}{x_{0,t} y_{Y,0}}\\
    &= \frac{y_{Y,0}}{y_{Y,0}+2\kappa_p} \frac{2 d_Y^0}{x_{0,t} y_{Y,0}} \left[ \frac{y_{Y,0}+2 \kappa_p}{y_{Y,0}+\kappa_p} \hat{\alpha}_p^{opt} -\frac{M_p}{y_{Y,0}+\kappa_p} \right],
\end{align}
where $\overline{\alpha}_p^{opt} \epsilon \equiv \overline{\alpha}_p^{opt}-1$.

Combining the expressions above, we obtain
\begin{equation}
    (\epsilon_{Y,t}^T)^{-1} 
    =(1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{y_{Y,0}+2\kappa_p} \frac{(1-x_c)}{x_a-x_c} \left[ \overline{\alpha}_{p,t}^{opt}- \overline{\alpha}_{p,t}+\frac{\kappa_p(\overline{\alpha}_{p,t}^{opt}-\overline{\alpha})}{y_{Y,0}+\kappa_p}  +\frac{(\gamma-1) \sigma_p \sigma^\top}{y_{Y,0}+\kappa_p} \right] + \mathcal{O}(\epsilon^2),
\end{equation}
using the fact that $\overline{\alpha} = 1 + \hat{\alpha}\epsilon$.
...
\section{Production and monetary policy}\label{sec:production_and_monetary_policy}

This appendix describes an extension of the baseline model described in Section~\ref{sec:model} to allow for production and monetary policy.
The response of interest rates played a crucial role in the determination of aggregate market elasticity, but, in practice, the interest rate is under control of the central bank. 
In this extension, we show that a central bank following an inflation-targeting policy would choose the same interest rate as the one implied by the baseline model.

We describe next a version of the model with production and monetary policy. 
We focus only on the main changes relative to the baseline model. 
We first describe the version of the model with production and flexible prices. 
We then describe the version of the model with sticky prices and monetary policy.

\subsection{The flexible-price version}

\paragraph{Firm's problem.} As it is standard in the literature, production is carried by a continuum of monopolistically competitive firms indexed by $i \in [0,1]$.
Firm $i$'s production function is given by
...
\end{equation}
where $\mu$ is the drift and $\sigma \in \mathbb{R}^{1\times 3}$ is the volatility. 
As in the baseline model, we assume that only the first entry of $\sigma$ is non-zero.

Using the production function \eqref{eq:production_function} to eliminate $L_{i,t}$, we can write the firm's problem as follows:
\begin{equation}
    \max_{P_{i,t}} P_{i,t} \tilde{Y}_{i,t} - \mathcal{W}_{i,t} \left(\frac{\tilde{Y}_{i,t}}{A_t}\right)^{\frac{1}{1-\alpha}},
\end{equation}
subject to the demand function:
\begin{equation}\label{eq:demand_function}
    \tilde{Y}_{i,t} = \left( \frac{P_{i,t}}{P_t} \right)^{-\phi} \tilde{Y}_t,
\end{equation}
where $\mathcal{W}_t$ is the nominal wage, $P_t = \left[ \int_{i=0}^1 P_{i,t}^{1-\phi} di \right]^{\frac{1}{1-\phi}}$ is the aggregate price level and $\phi$ is the elasticity of substitution between goods.

The first-order condition gives the usual pricing condition, where the price equals a markup over the marginal cost:
\begin{equation}
    P_{i,t} = \frac{\phi}{\phi-1} \frac{\mathcal{W}_{i,t}}{1-\alpha} \frac{\tilde{Y}_{i,t}^{\frac{\alpha}{1-\alpha}}}{A_t^{\frac{1}{1-\alpha}}}.
\end{equation}

In a symmetric equilibrium, all firms charge the same price, so $P_{i,t} = P_t$ for all $i$. 
We can then write the pricing condition as:     
\begin{equation}
    (1-\phi^{-1})(1-\alpha) A_t L_{i,t}^{-\alpha} = \frac{\mathcal{W}_t}{P_t}.
\end{equation}

\paragraph{Household's problem.} 
As in the baseline model, we have three types of households, $j = 0,1,2$, which differ in their risk aversion and portfolio constraints.
Following \citet{BigioSilvaZilberman2025}, we assume that households have Epstein-Zin preferences over a GHH composite of consumption and labor. 
...
\begin{equation}\label{eq:inv-problem-appendix}
    V_{j,t} = \max_{\tilde{C}_{j,t},L_{j,t},\alpha_{j,t}} 
    \mathbb{E}_t \left[ \int_t^\infty f_j(\tilde{C}_{j,s}- \xi_s L_{j,s},V_{j,s})\, ds \right], \qquad s.t.
\end{equation}
\begin{equation}\label{eq:budget_constraint-appendix}
    \frac{d W_{j,t}}{W_{j,t}} = \left[r_t + \pi_{t}^\top \,\alpha_{j,t}  - \frac{\tilde{C}_{j,t} - \mathcal{W}_t / P_t L_{j,t}}{W_{j,t}}\right] dt 
    + \sigma_{j,t}\, dZ_t,
\end{equation}
with $W_{j,t} \geq 0$ and $\alpha_{j,t} \in \Omega_{j,t}$,
where $\sigma_{j,t} = \alpha_{j,t}^ \top \Sigma_{t}$ is the household's risk exposure,
$W_{j,t}$ denotes the household's wealth, $\Omega_{j,t}$ is the investment set for type-$j$ household, and $\xi_t$ controls the disutility of labor supply.

Define \textit{net consumption} as $C_{j,t} = \tilde{C}_{j,t} - \xi_s L_{j,s}$. We can then rewrite the budget constraint as
\begin{equation}
    \frac{d W_{j,t}}{W_{j,t}} = \left[r_t + \pi_{t}^\top \,\alpha_{j,t}  - \frac{C_{j,t}}{W_{j,t}} + \frac{\mathcal{W}_t/P_t -\xi_t}{W_{j,t}} L_{j,t}\right] dt 
    + \sigma_{j,t}\, dZ_t.
\end{equation}
...
We assume that labor disutility proportional to aggregate producitivity:
\begin{equation}
    \xi_t = \xi A_t.
\end{equation}
This acts as an income effect on labor supply that ensures that labor supply is stationary despite the growth of aggregate producitivity.

\paragraph{Central bank.} We consider a cashless economy with a central bank that sets the nominal interest rate $i_t$. 
We assume that prices are predetermined, consistent with the behavior under sticky prices, so the nominal interest rate is given by $i_t = r_t + \mu_{P,t}$, where $\mu_{P,t}$ is the drift of the aggregate price level.
We consider a central bank that implements zero inflation at all periods, so $\mu_{P,t} = 0$, which requires setting the nominal interest rate to the (natural) risk-free rate.

\paragraph{Equilibrium in labor markets and production.} The market clearing condition for labor is given by
\begin{equation}
...
\begin{equation}
    L_t = \left(  \frac{(1-\alpha)(1-\phi^{-1})}{\xi}\right)^{\frac{1}{\alpha}} \equiv \overline{L}.
\end{equation}
...
\begin{equation}
    Y_t = \alpha A_t \overline{L}^\alpha.
\end{equation}
...
Hence, the equilibrium of the flexible-price economy is identical to the equilibrium of the baseline endowment economy considered in Section~\ref{sec:model}, once the nominal interest rate is given by $i_t = r_t$ and the central bank implements zero inflation at all periods.


\subsection{The sticky-price version}

Suppose now that firms are subject to Rotemberg adjustment costs in pricing. In this case, the firm's problem is given by
\begin{equation}
    Q_{i,t}(P_{i,t}) = \max_{[\pi_{i,s}]_{s\geq t} }\mathbb{E}_t \left[ \int_t^\infty \frac{\Lambda_{s}}{\Lambda_t} \left( \frac{P_{i,s}}{P_s} \tilde{Y}_{i,s} - \frac{\mathcal{W}_{s}}{P_s} \left(\frac{\tilde{Y}_{i,s}}{A_s}\right)^{\frac{1}{1-\alpha}}  - \frac{\varphi}{2} \pi_{i,s}^2\right)ds  \right].
\end{equation}
subject to the demand function \eqref{eq:demand_function} and $\tilde{P}_{i,t} = \pi_{i,t}P_{i,t}$, given the initial price $P_{i,t}$.

We assume that the adjustment cost is rebated back to shareholders, so they affect the firm's decision, but it does not represent a real resource cost. 
This assumption is not essential for the results, but it simplifies the analysis.

The next lemma provides a characterization of the equilibrium inflation rate in this economy. 

\begin{lemma}
    In a symmetric equilibrium where $P_{i,0} = P_0$, the equilibrium inflation rate $\pi_t \equiv \frac{\dot{P}_t}{P_t}$ is given by
    \begin{equation}\label{eq:inflation_rate}
        \pi_{t} = \mathbb{E}_t\left[ \int_t^\infty \frac{\Lambda_s}{\Lambda_t} \frac{\phi-1}{\varphi}\frac{\tilde{Y}_{t}}{P_t} \left( \frac{\phi}{\phi-1} \frac{\mathcal{W}_{t}}{P_{t}} \frac{\tilde{Y}_{t}^{\frac{\alpha}{1-\alpha}}}{(1-\alpha) A_s^{\frac{1}{1-\alpha}}}-1\right) ds\right],
    \end{equation}
    where we imposed that $P_{i,t} = P_t$, so we obtain a symmetric solution for all firms.
\end{lemma}

\begin{proof}
The value function $Q_{i,t}(P_{i,t}) = Q_i(P_{i,t}; \hat{X}_t)$ is a function of the aggregate state variable $\hat{X}_t$.

The HJB equation is given by
\begin{equation}
    0 = \left[\frac{P_{i,t}}{P_t} \tilde{Y}_{i,t} - \frac{\mathcal{W}_{t}}{P_t} \left(\frac{\tilde{Y}_{i,t}}{A_t}\right)^{\frac{1}{1-\alpha}}  - \frac{\varphi}{2} \pi_{i,t}^2 \right] dt+ \frac{\mathbb{E}_t[d(\Lambda_t Q_{i,t})]}{\Lambda_t}.
\end{equation}

More explicitly, we can write the HJB equation as
\begin{align}
    0 &= \frac{P_{i,t}}{P_t} \tilde{Y}_{i,t} - \frac{\mathcal{W}_{t}}{P_t} \left(\frac{\tilde{Y}_{i,t}}{A_t}\right)^{\frac{1}{1-\alpha}}  - \frac{\varphi}{2} \pi_{i,t}^2+ \frac{\partial Q_{i,t}}{\partial P_{i}} \pi_{i,t} P_{i,t}  + \frac{\partial Q_{i,t}}{\partial \hat{X}_t^\top} \mu_{\hat{X},t} + \frac{1}{2}\sum_{k=1}^d  \sigma_{\hat{X},K,t}^\top \frac{\partial^2  Q_{i,t}}{\partial \hat{X}_t\partial \hat{X}_t^\top} \sigma_{\hat{X},K,t}\\
    &-r_t Q_{i,t} - \eta_t \sigma_{Q_i,t}^\top.
\end{align}

The first-order condition gives the following pricing condition:
\begin{equation}
    \varphi \pi_{i,t} = \frac{\partial Q_{i,t}(P_{i})}{\partial P_{i}} 
\end{equation}

The envelope condition with respect to $P_{i,t}$ is given by
\begin{equation}
0 = (1-\phi)\frac{\tilde{Y}_{i,t}}{P_t} \left[ 1 - \frac{\phi}{\phi-1} \frac{\mathcal{W}_{t}}{P_{i,t}} \frac{\tilde{Y}_{i,t}^{\frac{\alpha}{1-\alpha}}}{(1-\alpha) A_t^{\frac{1}{1-\alpha}}}\right] dt  + \frac{\mathbb{E}_t[d(\Lambda_t \frac{\partial Q_{i,t}}{\partial P_{i,t}})]}{\Lambda_t}.
\end{equation}

Multiplying both sides by $\varphi^{-1}$ and using the first-order condition, we obtain
The envelope condition with respect to $P_{i,t}$ is given by
\begin{equation}
0 = \varphi^{-1}(1-\phi)\frac{\tilde{Y}_{i,t}}{P_t} \left[ 1 - \frac{\phi}{\phi-1} \frac{\mathcal{W}_{t}}{P_{i,t}} \frac{\tilde{Y}_{i,t}^{\frac{\alpha}{1-\alpha}}}{(1-\alpha) A_t^{\frac{1}{1-\alpha}}}\right] dt  + \frac{\mathbb{E}_t[d(\Lambda_t \pi_{i,t})]}{\Lambda_t}.
\end{equation}
...
Equation~\eqref{eq:inflation_rate} corresponds to the non-linear version of the Phillips curve in our sticky-price model. 
Crucially, it depends on how much the real marginal cost is expected to deviate from its flexible-price level.


\paragraph{Central bank and the zero-inflation equilibrium.} We assume that the central bank adopts the following Taylor rule:
\begin{equation}
    i_t = \varrho_t + \phi_\pi \pi_t,
\end{equation}
where $\phi_\pi > 1$ is the Taylor coefficient on inflation and $\varrho_t$ is a time-varying intercept. 

Let $r_t^n$ be the natural rate of interest, that is, the real interest rate in the flexible-price economy.
We assume that the time-varying intercept coincides with the natural rate of interest:  $\varrho_t = r_t^n$.
This implies that the inflation rate is equal to zero in our sticky-price economy.
...
\begin{equation}
    \frac{\phi}{\phi-1} \frac{\mathcal{W}_{t}}{P_{t}} \frac{\tilde{Y}_{t}^{\frac{\alpha}{1-\alpha}}}{(1-\alpha) A_s^{\frac{1}{1-\alpha}}}-1 = 0.
\end{equation}

This coincides with the pricing condition for the flexible-price economy, so the equilibrium labor supply is given by $L_t = \overline{L}$ and aggregate net output is given by $Y_t = \alpha A_t \overline{L}^\alpha$.
This implies that $Y_t$ follows the process:
\begin{equation}
    \frac{dY_t}{Y_t} = \mu dt + \sigma dZ_t.
\end{equation}

The interest rate in this economy will then coincide with the natural rate of interest: $i_t = r_t^n$, which requires the intercept $\varrho_t$ to be equal to the natural rate of interest: $\varrho_t = r_t^n$.




\section{The second-order demand system}

Consider next the second-order approximation of the demand system. 
We focus on the case where there is no preference heterogeneity and no leverage constraints.

\paragraph{Diffusion and drift of price-dividend ratio.} The drift of $p_{Y,t}$ is given by
\begin{equation}
    \sigma_{p_Y} = \frac{p_{Y,\overline{\alpha}_p}}{p_{Y}} \sigma_{p} + \frac{p_{Y,x}}{p_{Y}} \sigma_{x}.
\end{equation}

The price-dividend ratio is given by
\begin{equation}
    p_{Y}(x, \overline{\alpha}_p) =p_{Y}^*+ p_{Y,2}(x, \hat{\alpha}_p)\epsilon^2 + \mathcal{O}(\epsilon^3),
\end{equation}
using the fact that $p_{Y,1} = 0$ in the absence of preference heterogeneity.

The expression above implies that $p_{Y,x} = \mathcal{O}(\epsilon^2)$.
As $\sigma_{x} = \mathcal{O}(\epsilon)$, we have that $\frac{p_{Y,x}}{p_{Y}} \sigma_{x} = \mathcal{O}(\epsilon^3)$.
We can then ignore this term in the diffusion of $p_{Y,t}$ up to second order.

Taking the derivative of $p_{Y}$ with respect to $\hat{\alpha}_p$, we obtain
\begin{equation}
    \frac{p_{Y,\overline{\alpha}_p}}{p_{Y}} \frac{\partial \overline{\alpha}_p}{\partial \hat{\alpha}_p}= \frac{p_{Y,2, \hat{\alpha}_p}}{p_{Y,0}} \epsilon^2  +\mathcal{O}(\epsilon^3)\Rightarrow \frac{p_{Y,\overline{\alpha}_p}}{p_{Y}} = \frac{p_{Y,2, \hat{\alpha}_p}}{p_{Y,0}} \epsilon  +\mathcal{O}(\epsilon^2),
\end{equation}
as $\frac{\partial \overline{\alpha}_p}{\partial \hat{\alpha}_p} =\epsilon$.

Hence, the diffusion of $p_{Y,t}$ can be written as
\begin{equation}
    \sigma_{p_Y} =  \frac{p_{Y,2, \hat{\alpha}_p}}{p_{Y,0}} \hat{\sigma}_{p} \epsilon^2 + \mathcal{O}(\epsilon^3). 
\end{equation}

The price-dividend ratio takes the form
\begin{equation}
    \hat{p}_{Y,t} =  \frac{a_{Y}(x_t)}{2}(\overline{\alpha}_{p,t}-1)^2 + b_{Y}(x_t) (\overline{\alpha}_{p,t}-1) + c_{Y}(x_t)+ \mathcal{O}(\epsilon^3),
\end{equation}
where $a_{Y}(x_t)$, $b_{Y}(x_t)$, and $c_{Y}(x_t)$ are functions to be determined.

The derivative of the price-dividend ratio with respect to $\overline{\alpha}_p$ is given by
\begin{equation}
    \frac{\partial \hat{p}_{Y,t}}{\partial \overline{\alpha}_p} = a_{Y}(x_t) (\overline{\alpha}_{p,t}-1) + b_{Y}(x_t)+ \mathcal{O}(\epsilon^2),
\end{equation}

The diffusion of the price-dividend ratio is given by
\begin{equation}
    \sigma_{p_Y} = \left[ a_Y(x_t) (\overline{\alpha}_{p,t}-1) + b_Y(x_t) \right] \sigma_p +\mathcal{O}(\epsilon^3).
\end{equation}
...
\begin{equation}
    \mu_{p_Y} = [a_Y(x) (\overline{\alpha}_{p,t}-1) + b_Y(x)] \kappa_p(1-\overline{\alpha}_p) + \frac{1}{2} a_Y(x) \sigma_p \sigma_p^\top + \mathcal{O}(\epsilon^3).
\end{equation}

\paragraph{The demand for the risky asset.} The demand for the risky asset is given by  
\begin{equation}    
...
\begin{equation}
    \alpha_{j,Y} =  \frac{\sigma_{j} \sigma_{R_Y}^\top}{\sigma_{R_Y} 
    \sigma_{R_Y}^\top} =  \frac{(\sigma+\sigma_{j,1} \epsilon + \sigma_{j,2} \epsilon^2) (\sigma + \sigma_{p_Y,2} \epsilon^2)^\top}{(\sigma + \sigma_{p_Y,2} \epsilon^2)(\sigma + \sigma_{p_Y,2} \epsilon^2)^\top} = 1 + \frac{\sigma_{j,1} \sigma^\top}{\sigma \sigma^\top} \epsilon + \frac{(\sigma_{j,2} -\sigma_{p_Y,2})\sigma^\top}{\sigma \sigma^\top} \epsilon^2 + \mathcal{O}(\epsilon^3).
\end{equation}

The risk exposure of active investors is given by
\begin{equation}
    \sigma_{j,t} = \frac{\eta_0}{\gamma} + \frac{\eta_1}{\gamma} \epsilon + \left[\frac{\eta_2}{\gamma} + \frac{1-\gamma^{-1}}{\psi-1} \sigma_{c_j,2}\right] \epsilon^2 + \mathcal{O}(\epsilon^3),
\end{equation}
and the risk exposure of the passive investor is given by
\begin{equation}
    \sigma_{0,t} = \sigma + \hat{\alpha}_p \sigma \epsilon + \sigma_{p_Y,2} \epsilon^2 + \mathcal{O}(\epsilon^3).
\end{equation}

Combining the expressions above, we obtain the demand for the risky asset for active investors:
\begin{equation}
    q_{j,t} = \frac{\eta_t \sigma^\top}{\gamma \|\sigma\|^2} + \left[\frac{1-\gamma^{-1}}{\psi-1} \sigma_{c_j,t} - \sigma_{p_Y,t} \right] \frac{\sigma^\top}{\|\sigma\|^2} + \mathcal{O}(\epsilon^3).
\end{equation}
The demand for the risky asset for passive investors is given by
\begin{equation}
    q_{0,t} = \overline{\alpha}_{p,t} - 1 + \mathcal{O}(\epsilon^3).
\end{equation}

Next, we eliminate the market price of risk from the demand for the risky asset. 
...
\begin{equation}
    \left[\eta_1\epsilon + \eta_2\epsilon^2 \right] \sigma^\top = \left[-\frac{1}{p_{Y,0}^2}p_{Y,1} - r_1\right]\epsilon + \left[- \frac{p_{Y,2}}{p_{Y,0}^2} + \frac{p_{Y,1}^2}{p_{Y,0}^3} -r_2+ \mu_{p_Y,2} +(1-\gamma) \sigma \sigma_{p_Y,2}^\top \right]\epsilon^2.
\end{equation}

Using the fact that $p_{Y,1} =0$ in the absence of preference heterogeneity, we obtain
\begin{equation}
    q_{j} = -\frac{p_{Y,2}\epsilon^2}{p_{Y,0}^2 \gamma \|\sigma\|^2} - \frac{r_1\epsilon + r_2\epsilon^2}{\gamma \|\sigma\|^2} + \left[ \mu_{p_Y,2} +(1-2\gamma) \sigma_{p_Y,2}\sigma^\top + \frac{\gamma-1}{\psi-1} \sigma_{c_j,2} \sigma^\top\right]\frac{\epsilon^2}{\gamma\|\sigma\|^2} + \mathcal{O}(\epsilon^3).
\end{equation}
...
\begin{equation}
    q_{j,t} = - \zeta_{j,p} \hat{p}_{Y,t} - \zeta_{j,r} \hat{r}_t + f_{j,t} + \mathcal{O}(\epsilon^3),
\end{equation}
...
\paragraph{Consumption-wealth ratio.} The aggregate consumption-wealth ratio $c_{t} \equiv \sum_{j=0}^2 x_{j,t} c_{j,t}$ is given by
\begin{equation}
    c_{t} = \psi \rho + (1-\psi) \left[ r_t + \eta_t \sigma_{R_Y,t}^\top - \frac{\gamma}{2} \sum_{j=0}^2 x_{j,t} \|\sigma_{j,t}\|^2 \right] + \sum_{j=0}^2 x_{j,t} \left[\mu_{c_j,t} + (1-\gamma) \sigma_{c_j,t} \sigma_{R_Y,t}^\top + \frac{\psi - \gamma}{1-\psi} \frac{\|\sigma_{c_j,t}\|^2}{2}\right].
\end{equation}

The risk exposure of the passive investor is given by $\sigma_{0,t} = \overline{\alpha}_{p,t} \sigma_{R_Y,t}$. From market clearing, the risk exposure of active investors is given by $\sigma_{j,t} = \frac{1-x_{0,t}\overline{\alpha}_{p,t}}{1-x_{0,t}} \sigma_{R_Y,t}$. Hence, the risk exposure of both group of investors is proportional to $\sigma_{R_Y,t}$.
We can then write the consumption-wealth ratio as follows:
\begin{equation}
    c_{t} = \psi \rho + (1-\psi) \left[ r_t + \eta_t \sigma_{R_Y,t}^\top - \frac{\gamma\|\sigma_{R_Y,t}\|^2}{2} \sum_{j=0}^2 x_{j,t} \alpha_{j,Y,t}^2 \right] + \sum_{j=0}^2 x_{j,t} \left[\mu_{c_j,t} + (1-\gamma) \sigma_{c_j,t} \sigma_{R_Y,t}^\top + \frac{\psi - \gamma}{1-\psi} \frac{\|\sigma_{c_j,t}\|^2}{2}\right].
\end{equation}

Expanding the expression above up to second order, we obtain
\begin{equation}
    c_{2} = (1-\psi) \left[ r_2 + \eta_2 \sigma^\top - \frac{\gamma\|\sigma\|^2}{2} \frac{x_0}{x_a}(\overline{\alpha}_{p,t}-1)^2 \right] - \left[\mu_{p_Y,2} + (1-\gamma) \sigma_{p_Y,2} \sigma^\top \right].
\end{equation}
...
Combining the expressions above, we obtain
\begin{equation}
    c_{2} = (1-\psi) \left[ - \frac{p_{Y,2}}{p_{Y,0}^2}  + \mu_{p_Y,2} + (1-\gamma) \sigma_{p_Y,2} \sigma^\top - \frac{\gamma\|\sigma\|^2}{2} \frac{x_0}{x_a}(\overline{\alpha}_{p,t}-1)^2 \right] - \left[\mu_{p_Y,2} + (1-\gamma) \sigma_{p_Y,2} \sigma^\top \right].
\end{equation}

The consumption-wealth ratio takes the form
\begin{equation}
    \hat{c}_{t} = \chi_{0,t} + \chi_p \hat{p}_{Y,t} + \mathcal{O}(\epsilon^3),
\end{equation}
where
\begin{equation}    
...
\paragraph{The aggregate market elasticity.} 
Using the market clearing condition for goods, we have
\begin{equation}
    c_t = - y^* \hat{p}_{Y,t} + \mathcal{O}(\epsilon^3) \Rightarrow \hat{p}_{Y,t} = - \frac{\chi_{0,t}}{\chi_p + y^*} + \mathcal{O}(\epsilon^3).
\end{equation}
...
\begin{align}
    \hat{p}_{Y,t} &= 
    -\left[(1-\psi^{-1}) \gamma\|\sigma\|^2 \frac{x_0}{x_a} + 2\kappa_p a_Y(x) \right] \frac{(\overline{\alpha}_{p,t}-1)^2}{2 y^*} 
 - \frac{1}{y^*} \left[b_Y(x) \kappa_p +(\gamma-1)  a_Y(x_t)\sigma_p \sigma^\top \right] (\overline{\alpha}_{p,t}-1) \\
 &+ \frac{1}{y^*} \left[ \frac{1}{2} a_Y(x) \sigma_p \sigma_p^\top + (1-\gamma) b_Y(x_t) \sigma_p \sigma^\top \right]\\
\end{align}
...
The derivative of $\hat{p}_{Y,t}$ with respect to $\overline{\alpha}_p$ is given by
\begin{equation}    
    \frac{\partial \hat{p}_{Y,t}}{\partial \overline{\alpha}_p} = a_Y(x) (\overline{\alpha}_{p,t}-1) + b_Y(x).
\end{equation}
Using the expression above, we obtain
\begin{equation}
  \epsilon_{Y,t}^{-1} \equiv  \frac{1}{x_{0,t}}\frac{\partial \hat{p}_{Y,t}}{\partial \overline{\alpha}_p} =  \frac{1-\psi^{-1}}{y^* + 2\kappa_p} \frac{\gamma\|\sigma\|^2}{x_a} \left[ 1- \overline{\alpha}_{p,t} + \frac{\gamma-1}{y^* + \kappa_p}\sigma_p \sigma^\top\right] + \mathcal{O}(\epsilon^2).
\end{equation}

\paragraph{Endogenous volatility and hedging demand.} The volatility of the price-dividend ratio is given by
\begin{equation}
    \sigma_{p_Y,t} = x_{0,t} \epsilon_{Y,t}^{-1} \sigma_p + \mathcal{O}(\epsilon^3).
\end{equation}

Let $c_{j,t}^0$ denote the consumption-wealth ratio in the fixed-$\hat{\alpha}_p$ economy.
In this case, we can write $y_{Y,2}^0 = d_Y \hat{\alpha}_p^2$, and the second-order correction to the consumption-wealth ratio of active investor $j=1,2$ is given by
\begin{equation}
...
\begin{equation}
    \sigma_{c_j,t} = - \left[1 - \frac{1}{x_a} \psi \right] x_{0,t} \epsilon_{Y,t}^{-1} \sigma_p + \mathcal{O}(\epsilon^3),
\end{equation}
and the diffusion of the consumption-wealth ratio for passive investor is given by
\begin{equation}
    \sigma_{c_0,t} =  -\left[1 + \frac{1}{x_0} \psi \right] x_{0,t} \epsilon_{Y,t}^{-1} \sigma_p + \mathcal{O}(\epsilon^3).
\end{equation}

We can then write the hedging demand for active investor $j=1,2$ as follows:
\begin{equation}
    h_{j,t} = \frac{1-\gamma_j^{-1}}{\psi-1} \sigma_{c_j,t} = H_{a,t} x_{0,t} \epsilon_{Y,t}^{-1} \sigma_p + \mathcal{O}(\epsilon^3),
\end{equation}
where $H_{a,t} = \frac{1-\gamma^{-1}}{\psi-1} \left[\frac{\psi }{x_{a,t}} -1\right]$.


\paragraph{Market price of risk and risk premium.} The market price of risk is given by
\begin{equation}
    \eta_t = \gamma \sigma \chi_{a,t}+ \left[1 - H_{a,t} \right] \gamma x_{0,t} \epsilon_{Y,t}^{-1} \sigma_p + \mathcal{O}(\epsilon^3),
\end{equation}
where $\chi_{a,t} = \frac{1-x_{0,t}\overline{\alpha}_{p,t}}{1-x_{0,t}}$ is the aggregate risky exposure that must be absorbed by active investors per unit of active wealth, and $h_{a,t}$ is the average hedging demand of active investors, adjusted for leverage constraints.

The risk premium is given by
\begin{equation}
    \pi_{Y,t} = \gamma \|\sigma\|^2 \chi_{a,t}+ \left[2 - H_{a,t} \right] \gamma x_{0,t} \epsilon_{Y,t}^{-1} \sigma_p \sigma^\top+ \mathcal{O}(\epsilon^3).
\end{equation}

\paragraph{Interest rate.} 
The interest rate follows from the pricing condition for the aggregate claim,
\begin{equation}
    \frac{1}{p_{Y,t}}
    = r_t + \eta_t\sigma_{R_Y,t}^\top
    -\left(\mu+\mu_{p_Y,t}+\sigma\sigma_{p_Y,t}^\top\right).
\end{equation}
Thus,
\begin{equation}
    r_t
    = y_{Y,t}+\mu+\mu_{p_Y,t}+\sigma\sigma_{p_Y,t}^\top-\pi_{Y,t},
\end{equation}
where $y_{Y,t}=1/p_{Y,t}$ and $\pi_{Y,t}=\eta_t\sigma_{R_Y,t}^\top$.

From the market clearing condition for goods, we have   
\begin{equation}
\chi_{0,t} + (\chi_p + y^*) \hat{p}_{Y,t} = \mathcal{O}(\epsilon^3),
\end{equation}
which can be written as
\begin{equation}
    (1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{2} \frac{x_0}{x_a}(\overline{\alpha}_{p,t}-1)^2 = \left[ \mu_{p_Y,t} + (1-\gamma) \sigma_{p_Y,t} \sigma^\top- y^* \hat{p}_{Y,t} \right] +\mathcal{O}(\epsilon^3).
\end{equation}

We can then write the interest rate as follows:
\begin{equation}
    \hat{r}_t = (1-\psi^{-1}) \frac{\gamma\|\sigma\|^2}{2} \frac{x_0}{x_a}(\overline{\alpha}_{p,t}-1)^2 - \hat{\eta}_t \sigma^\top + \mathcal{O}(\epsilon^3).
\end{equation}


\section{Details of Section~\ref{sec:quantitative}}

\subsection{Flow of Funds aggregation}\label{app:fof_aggregation}
...
\subsection{Numerical methodology}\label{app:numerical}
...
\subsubsection{Simplified economy}
