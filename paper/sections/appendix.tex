...
\begin{proof} 
    ...
    We can then focus on the reduced set of states $\hat{X} = (x_{1}, x_{2}, \hat{\alpha}_{p})$. 


    We assume that the portfolio share of passive investors is given by $\overline{\alpha}_{p} = 1 + \hat{\alpha}_{p} \epsilon$, where $\hat{\alpha}_{p}$ follows the following process independent of $\epsilon$:
    \begin{equation}
        d \hat{\alpha}_{p} = \kappa_p (\hat{\alpha} - \hat{\alpha}_{p}) dt + \sigma_p dZ_t.
    \end{equation}
    This implies that $\overline{\alpha} = 1+\hat{\alpha} \epsilon$ and the diffusion of $\overline{\alpha}_{p,t}$ is given by $\sigma_p \epsilon$.

    Notice this implies that $\mu_{\overline{\alpha}_{p}} = \mathcal{O}(\epsilon)$ and $\sigma_{\overline{\alpha}_{p}} = \mathcal{O}(\epsilon)$.
    Similarly, we have that $\mu_{x_{j}} = \mathcal{O}(\epsilon)$ and $\sigma_{x_{j}} = \mathcal{O}(\epsilon)$ for $j = 1,2$.
    Hence, we have that $\mu_{\hat{X}} = \mathcal{O}(\epsilon)$ and $\sigma_{\hat{X}} = \mathcal{O}(\epsilon)$.

    We are interested in the first-order correction terms. 
    For instance, the consumption-wealth ratio of investor $j$ can be written as
    \begin{equation}
    ...
    \end{equation}
    The first-order correction corresponds to $c_{j,1}(x, \hat{\alpha}_p)$.

    % \paragraph{The change of variables.}
    % To derive the first-order correction to the portfolio share of passive investors, it will be convenient to perform first a change of variables.
    % Instead of working with $\overline{\alpha}_{p} = 1 + \hat{\alpha}_{p} \epsilon$, we will work with the variable$\hat{\alpha}_{p}$, which capures the deviation from the benchmark portfolio share of passive investors.
    
    % % The portfolio share of passive investors is given by $\overline{\alpha}_{p,t} = 1 + \hat{\alpha}_{p,t} \epsilon$, where
    % % \begin{equation}
    % %     d \overline{\alpha}_{p,t} =  \kappa_p (\hat{\alpha} - \hat{\alpha}_{p,t}) \epsilon dt + \sigma_p \epsilon dZ_t,
    % % \end{equation}
    % % where $\overline{\alpha} = 1+\hat{\alpha} \epsilon$, so the process for $\hat{\alpha}_{p,t}$ is independent of $\epsilon$.
    % The process for $\hat{\alpha}_{p}$ is independent of $\epsilon$ and can be written as
    % \begin{equation}
    %     d \hat{\alpha}_{p} = \underbrace{\kappa_p (\hat{\alpha} - \hat{\alpha}_{p})}_{\mu_{\hat{\alpha}_{p}}} dt + \underbrace{\sigma_p}_{\sigma_{\hat{\alpha}_{p}}} dZ_t.
    % \end{equation}
    % This implies that $\overline{\alpha} = 1+\hat{\alpha} \epsilon$ and the diffusion of $\overline{\alpha}_{p,t}$ is given by $\sigma_{\overline{\alpha}_{p,t}} = \sigma_p \epsilon$.

    % The derivatives of the consumption-wealth ratios, and similarly for the price-dividend ratio, with respect to $\overline{\alpha}_{p,t}$ are given by
    % \begin{equation}
    %     \frac{\partial c_j}{\partial \hat{\alpha}_{p}} = \frac{\partial c_j}{\partial \overline{\alpha}_{p}} \frac{\partial \overline{\alpha}_{p}}{\partial \hat{\alpha}_{p}} = \frac{\partial c_j}{\partial \overline{\alpha}_{p}} \epsilon, \qquad \qquad 
    %     \frac{\partial^2 c_j}{\partial \hat{\alpha}_{p}^2} = \frac{\partial^2 c_j}{\partial \overline{\alpha}_{p}^2} \epsilon^2, 
    %      \qquad \qquad 
    %     \frac{\partial^2 c_j}{\partial \hat{\alpha}_{p} \partial x_j} = \frac{\partial^2 c_j}{\partial \overline{\alpha}_{p} \partial x_j} \epsilon, 
    % \end{equation}
    % and the drift and difussion of $\hat{\alpha}_{p,t}$ are given by   
    % \begin{equation}
    %     \mu_{\overline{\alpha}_{p}} = \mu_{\hat{\alpha}_{p}} \epsilon, \qquad \qquad 
    %     \sigma_{\overline{\alpha}_{p}} = \sigma_{\hat{\alpha}_{p}} \epsilon.
    % \end{equation}

    % This implies that we can write the terms in the drift and diffusion of the consumption-wealth ratios and the price-dividend ratio as functions of $\hat{\alpha}_{p}$ instead of $\overline{\alpha}_{p}$:
    % \begin{equation}
    %     \frac{\partial c_j}{\partial \overline{\alpha}_{p}} \sigma_{\overline{\alpha}_{p}} = \frac{\partial c_j}{\partial \hat{\alpha}_{p}} \sigma_{\hat{\alpha}_{p}}, \qquad 
    %     \frac{\partial^2 c_j}{\partial \overline{\alpha}_{p}^2} \sigma_{\overline{\alpha}_{p}}^2 = \frac{\partial^2 c_j}{\partial \hat{\alpha}_{p}^2} \sigma_{\hat{\alpha}_{p}}^2, \qquad 
    %     \frac{\partial^2 c_j}{\partial \overline{\alpha}_{p} \partial x_j} \sigma_{\overline{\alpha}_{p}} \sigma_{x_j} = \frac{\partial^2 c_j}{\partial \hat{\alpha}_{p} \partial x_j} \sigma_{\hat{\alpha}_{p}} \sigma_{x_j}.
    % \end{equation}

    % Hence, abusing notation, we can write the vector of states as $\hat{X} = (x_{1,t}, x_{2,t}, \hat{\alpha}_{p})$.
...
The consumption-wealth ratios and the price-dividend ratio for the aggregate claim are independent of $x_j$ at $\epsilon = 0$.
This implies that $c_{j,x_j}=\mathcal{O}(\epsilon)$ and $p_{Y,x_j}=\mathcal{O}(\epsilon)$ as these derivatives are zero at order zero.
Moreover, we will guess-and-verify that the derivatives of the consumption-wealth ratios and the price-dividend ratio with respect to $\overline{\alpha}_{p}$ are first-order in $\epsilon$:
\begin{equation}
    c_{j,\overline{\alpha}_{p}} = \mathcal{O}(\epsilon); \qquad \qquad p_{Y,\overline{\alpha}_{p}} = \mathcal{O}(\epsilon).
\end{equation}
This implies that $\mu_{c_j, \hat{X}} = \mathcal{O}(\epsilon)$ and $\mu_{p_Y, \hat{X}} = \mathcal{O}(\epsilon)$.

Using the fact that $c_{j,0}(x, \hat{\alpha}_p)$ is independent of $\overline{\alpha}_p$, we can write the derivative with respect to $\overline{\alpha}_p$ as follows:
\begin{equation}
    c_{j,\overline{\alpha}_{p}} = \frac{\partial c_{j,1}}{\partial \hat{\alpha}_{p}}  +\mathcal{O}(\epsilon).
\end{equation}
Hence, $c_{j,\overline{\alpha}_{p}} = \mathcal{O}(\epsilon)$ is equivalent to $\frac{\partial c_{j,1}}{\partial \hat{\alpha}_{p}} = 0$, that is, $c_{j,1}$ is independent of $\hat{\alpha}_{p}$.
A similar argument shows that $p_{Y,\overline{\alpha}_{p}} = \mathcal{O}(\epsilon)$ is equivalent to $p_{Y,1}$ being independent of $\hat{\alpha}_{p}$.
...
\end{equation}
where $\hat{n}_{j}$ is the Lagrange multiplier on the leverage constraint for investor $j$.
...
The risk premium is higher when the average risk aversion of active investors is higher, leverage constraints are binding, and active investors hold disproportionately more risk.
...
It\^o's lemma implies that the drift of the consumption-wealth ratios and the price-dividend ratio are given by
\begin{align}
    \mu_{c_j} &= \frac{c_{j,\hat{X}}}{c_j} \mu_{\hat{X}} + \sum_{k=1}^3 \sigma_{\hat{X},k}^\top \frac{c_{j,\hat{X} \hat{X}}}{c_j} \sigma_{\hat{X},k}, \\
    \mu_{p_Y} &= \frac{p_{Y,\hat{X}}}{p_Y} \mu_{\hat{X}} + \sum_{k=1}^3 \sigma_{\hat{X},k}^\top \frac{p_{Y,\hat{X} \hat{X}}}{p_Y} \sigma_{\hat{X},k},
\end{align}
where $\sigma_{\hat{X},k}$ is the $k$-th column of the matrix $\sigma_{\hat{X}}$.

Notice that $\mu_{c_j} = \mathcal{O}(\epsilon^2)$ and $\mu_{p_Y} = \mathcal{O}(\epsilon^2)$, as
As the first derivatives are $\mathcal{O}(\epsilon)$, and the drift and diffusion terms of $\hat{X}$ are $\mathcal{O}(\epsilon)$, so the two terms in the expressions above are $\mathcal{O}(\epsilon^2)$.
This implies that $\xi_j = \mathcal{O}(\epsilon^2)$.
...
\end{equation}
which is independent of $\hat{\alpha}_p$ as conjectured.
If $\psi>1$, an increase in the average risk aversion of the investors leads to a decrease in the price-dividend ratio of the aggregate claim.
...
\begin{equation}
    d \hat{\alpha}_p = \kappa_p ( \overline{\hat{\alpha}} - \hat{\alpha}_p )dt + \sigma_p d Z_t,
\end{equation}
so the drift of $\overline{\alpha}_p$ is $\mu_{\overline{\alpha}_p} = \kappa_p ( \hat{\alpha} - \hat{\alpha}_p )\epsilon$ and the diffusion is
$\sigma_{\overline{\alpha}_p} = \sigma_p \epsilon$.

\paragraph{Pricing individual claims.}
The pricing condition for asset $k$, $k = D,B$, is given by
\begin{equation}
    r + \sigma_{R_k} \eta^\top = \frac{s_t}{\tilde{p}_{k}} + \mu + \mu_{\tilde{p}_k} + \sigma \sigma_{\tilde{p}_k}^\top,
\end{equation}
where $\sigma_{R_k} = \sigma + \sigma_{p_k}$ and $(\mu_{p_k}, \sigma_{p_k})$ are given by Ito's lemma:
