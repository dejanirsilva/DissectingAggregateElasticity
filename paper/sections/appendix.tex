...
We can then write the demand for the risky asset for active investors as follows:
\begin{equation}
...
\begin{equation}
    \hat{p}_{Y,t} = a_{Y}(x) + b_{Y}(x) (\overline{\alpha}-1) + \frac{c_{Y}(x)}{2} (\overline{\alpha}-1)^2 + \mathcal{O}(\epsilon^3),
\end{equation}
