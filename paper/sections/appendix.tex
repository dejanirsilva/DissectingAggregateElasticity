...
\begin{lemma}\label{lemma:block_recursivity}
    ...
    \begin{align}
        ...
        \frac{1}{p_{Y,t}} &= r_t + \eta_{t}^\top\,(\sigma + \sigma_{p_Y,t}) - \big(\mu + \mu_{p_Y,t} + \sigma \, \sigma_{p_Y,t}^\top\big),        
\end{align}
...
\begin{align}
        ...
        + (1-\gamma_j) \sigma_{c_j} \sigma_{j}^\top 
        + \frac{\psi - \gamma_j}{1-\psi} \frac{\|\sigma_{c_j}\|^2}{2}\\
        \frac{1}{p_{Y}} &= r + \eta^\top\,\sigma_{R_Y} - \big(\mu + \mu_{p_Y} + \sigma \, \sigma_{p_Y}^\top\big),        
\end{align}
...
This implies that the risk exposure for the aggregate claim is given by
\begin{equation}
...
\paragraph{Pricing the dividend claim.}
...
\begin{align}
    \frac{d \Lambda_t}{\Lambda_t} = -r^* dt - \sigma dZ_t, \qquad \qquad 
    \frac{d Y_t}{Y_t} = \mu dt + \sigma dZ_t.
\end{align}
We can then write the SDF and the aggregate endowment as:
\begin{align}
...
\begin{equation}
    P_{D,t} = \int_t^\infty e^{-\delta_D (\tau-t)} \mathbb{E}_t\left[ s_\tau \right] d\tau Y_t,
\end{equation}
where $\delta_{D} = r^* - (\mu - \sigma \sigma^\top)$, using the fact that $Z_t$ cancels out in the product $\Lambda_t Y_t$.
...
\begin{equation}
    P_{D,t} = \left[ \frac{\overline{s}}{\delta_D} + \frac{s_t - \overline{s}}{\delta_D + \kappa_s} \right] Y_t
\end{equation}
...
\begin{equation}
    p_{D,0}(X) = \frac{1}{\delta_D + \kappa_s}\left[1 + \frac{\kappa_s}{\delta_D}  \frac{\overline{s}}{s_t}  \right].
\end{equation}
...
\paragraph{Risk exposure.} The risk exposure for the passive investor is given by
\begin{equation}
...
\paragraph{Market price of risk.} Expanding the market clearing condition for risky assets, we obtain
\begin{equation}
    \sum_{j=0}^2 x_{j} \sigma_{j,1} = 0
\end{equation}
using the fact that $\sigma_{R_Y,1} = 0$.

When the leverage constraint is not binding, we have
\begin{equation}
