...
Let $p_{D,t} = \frac{P_{D,t}}{Y_t}$ and $p_{B,t} = \frac{P_{B,t}}{Y_t}$ denote the normalized prices of the dividend and bond claims, respectively. The normalized price of the aggregate claim is $p_{Y,t} = \frac{P_{Y,t}}{Y_t}$.
...
\begin{equation}
    \frac{1}{p_{D,t}} = r_t + \sigma_{R_D,t} \eta_t^\top - \big(\mu + \mu_{p_D,t} + \sigma \, \sigma_{p_D,t}^\top\big),
\end{equation}
where
\begin{equation}
...
\begin{equation}
    \mu_{p_Y,2} = -\mu_{y_Y,2},
\end{equation}
up to terms of order higher than $\epsilon^2$, since $\sigma_{y_Y,t}=\mathcal{O}(\epsilon^2)$.
...
\begin{align}\label{eq:r2-second-order-simplified}
    ...
    - \pi_{Y,2}.
\end{align}
This expression separates the second-order interest-rate correction into the price-drift term, the intertemporal-hedging term, the second-order change in average risk compensation, and the second-order risk premium.
...
\begin{equation}
    F(X,\epsilon)
    \equiv
    \frac{W_0 (1+\hat{\alpha}_p \epsilon) - W_0}{P}
    =
