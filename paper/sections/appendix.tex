...
\subsection{The case with frictions}

We consider next the aggregate market elasticity in richer settings. The second-order correction for the dividend yield in the case where the portfolio share of passive investors is fixed takes the form:
\begin{equation}    
    y_{Y,2}^0 = a_y^0 + b_y^0 \overline{\alpha}_p + d_y^0 \overline{\alpha}_p^2.
\end{equation}

The inverse aggregate market elasticity is given by
\begin{equation}
    (\epsilon_{Y,t}^0)^{-1} = -\frac{1}{x_{0,t} y_{Y,0}^2} \frac{\partial y_{Y,2}^0}{\partial \hat{\alpha}_p} \epsilon + \mathcal{O}(\epsilon^2).
\end{equation}
