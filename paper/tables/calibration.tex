\small
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{0.55\linewidth}S[table-format=2.4,table-number-alignment=center]@{}}
	\toprule	
	& Parameter & \multicolumn{1}{c}{Choice}  \\
	\midrule 
	&\small \emph{Preferences \& distribution}  & \\
	\midrule		
	$\psi$         & EIS & 1.5   \\
	$\rho$         & Discount rate                  & 0.01 \\
	$\gamma_0$     & Risk aversion of passive investors               & 10.86    \\
    $\gamma_1$     & Risk aversion of unconstrained investors                & 25.52 \\ 
	$\gamma_2$     & Risk aversion of constrained investors                & 6.15 \\
    %$\kappa$       & Agents' entry/exit rate                  & 0.10  \\
    $\omega_0$    & Share of passive agents                & 0.50 \\
    $\omega_1$    & Share of unconstrained agents                & 0.25 \\
	$\omega_2$    & Share of constrained agents                & 0.25 \\
	\midrule 
	& \small \emph{Technology}  & \\
	\midrule 
	$\mu$          & Endowment growth rate                    & 0.018  \\
	$\sigma$       & Endowment volatility                     & 0.036  \\
    $\sigma_s$     & Dividend share volatility                & 0.11 \\ 
    $\bar{s}$      & Mean dividend share                      & 0.50 \\ 
    $\kappa_s$     & Mean reversion of dividend share         & 0.10 \\
	\midrule 
	&\small \emph{Passive demand}&  \\
	\midrule
	$\overline{\alpha}$ & Mean &  0.52	\\
    $\theta_p$              & Mean reversion parameter  &  0.75 	\\
    $\sigma_p$             & Passive demand shock exposures &  0.02	\\
    $\nu$                  & Corr. of passive demand and fundamental shock & 0.50 \\ 

	\midrule
	& \small \emph{Leverage constraints}  & \\
	\midrule 
	$\overline{\sigma}$ & Tightness of the leverage constraint &  0.11	\\
	\bottomrule
\end{tabular}

%γ_vec=[10., 9.368, 4.925, 3.212, 1.552],
%ψ=2.5,
%ρ_=0.02,
%μ=0.022,
%σ=0.035,
%θp=0.69,
%σp0_=0.1,
%σp1_=0.02,
%σ_hat=0.02,
%κ_=0.05,
%α_=.1,
%ω=0.90713995, 0.02905808, 0.02313385, 0.02031372, 0.02035439
